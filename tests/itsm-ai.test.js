...
const { rankIntent, scoreIntent, tokenize } = require('../src/services/virtualAgent.service');

// --- tokenize ---
assert(JSON.stringify(tokenize('Reset my password NOW!')) === JSON.stringify(['reset', 'my', 'password', 'now']), 'tokenize lowercases and strips punctuation');

// --- scoreIntent ---
assert(scoreIntent('status update please', ['status']) > 0, 'scoreIntent detects single keyword');
assert(scoreIntent('what is the status', ['where is', 'status']) > 0, 'scoreIntent detects phrase+word');
...
(async () => {
  ...
  for (const [text, expected] of cases) {
    ...
    assert(r.intent === expected, `rankIntent('${text}')=${r.intent} expected ${expected}`);
  }
  ...
  assert(typeof slaModule.predictBreach === 'function', 'slaPredictor exports predictBreach');
  assert(typeof slaModule.predictBreachMany === 'function', 'slaPredictor exports predictBreachMany');
