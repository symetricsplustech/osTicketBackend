...
async function setup() {
  // Create tenant
  const company = await Company.create({ name: 'Test Tenant', domain: 'test-task-engine.example.com' });
...
async function testAddComment() {
  ...
  assert(res.body.content === 'This is a test comment', 'comment content matches');
}
...
async function testGetAllowedTransitions() {
  ...
  assert(res.body.allowed.includes('closed'), 'resolved can transition to closed');
}
