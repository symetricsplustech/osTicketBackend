...
function selectBestAgent(candidates, skills = [], strategy = "round_robin") {
  ...
  const available = candidates.filter((a) =>
    ["available", "online", "busy"].includes(a.presence),
  );
  ...
  switch (strategy) {
    case "round_robin":
      return pool.sort(
        (a, b) => (a.ticketCount || 0) - (b.ticketCount || 0),
      )[0];
    case "least_workload":
      return pool.sort(
        (a, b) => (a.ticketCount || 0) - (b.ticketCount || 0),
    ...
    case "weighted":
      return pool.sort(
        (a, b) => scoreAgent(b, skills) - scoreAgent(a, skills),
      )[0];
    default:
      return pool[0];
  }
...
assert(
  matchesCondition(
    { field: "category", operator: "in", value: ["software"] },
    ticket,
  ) === false,
  "in no match",
);
assert(
  matchesCondition(
    { field: "category", operator: "not_in", value: ["software"] },
    ticket,
  ) === true,
  "not_in match",
);
...
assert(
  matchesCondition({ field: "impact", operator: "gte", value: 3 }, ticket) ===
    true,
  "gte match",
);
assert(
  matchesCondition({ field: "impact", operator: "lte", value: 3 }, ticket) ===
    true,
  "lte match",
);
assert(
  matchesCondition({ field: "impact", operator: "gt", value: 2 }, ticket) ===
    true,
  "gt match",
);
assert(
  matchesCondition({ field: "impact", operator: "lt", value: 5 }, ticket) ===
    true,
  "lt match",
);
assert(
  matchesCondition(
    { field: "source", operator: "starts_with", value: "em" },
    ticket,
  ) === true,
  "starts_with match",
);
assert(
  matchesCondition(
    { field: "source", operator: "ends_with", value: "il" },
    ticket,
  ) === true,
  "ends_with match",
);
assert(
  matchesCondition(
    { field: "missing", operator: "is_empty", value: undefined },
    ticket,
  ) === true,
  "is_empty match",
);
assert(
  matchesCondition(
    { field: "priority", operator: "is_not_empty", value: "p1" },
    ticket,
  ) === true,
  "is_not_empty match",
);
assert(
  matchesCondition(
    { field: "unknown_op", operator: "bad_op", value: "x" },
    ticket,
  ) === false,
...
assert(
  matchesConditions({ matchAll: true, rules: [] }, ticket) === true,
...
const agent3 = {
  ...
  capacity: 10,
  presence: "available",
  skills: ["windows", "networking", "security"],
};
...
assert(
  score1 > score2,
  `agent1 (${score1}) > agent2 (${score2}): less workload, available`,
);
assert(
  score3 > score1,
  `agent3 (${score3}) > agent1 (${score1}): more skills, less workload`,
);
...
assert(
  rr.name === "B",
  "round_robin: picks fewest tickets among available (B=2)",
);

const ll = selectBestAgent(agents, [], "least_workload");
assert(
  ll.name === "B",
  "least_workload: picks fewest tickets among available (B=2)",
);

const sb = selectBestAgent(agents, ["windows", "linux"], "skill_based");
assert(
  sb.name === "B" || sb.name === "C",
  "skill_based: picks best skill match",
);

const av = selectBestAgent(agents, [], "availability");
assert(
  av.name === "A" || av.name === "B",
  "availability: picks available agent",
);
