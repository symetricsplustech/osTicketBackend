...
function recomputeInstanceStatus(steps) {
  ...
  if (statuses.every((s) => s === "approved" || s === "skipped"))
    return "approved";
...
function isSequentialComplete(steps, currentStep) {
  return steps
    .filter((s) => s.stepNumber < currentStep)
...
const VALID_TIMEOUT_ACTIONS = [
  "auto_approve",
  "auto_reject",
  "notify_admin",
  "escalate",
];
const VALID_ESCALATION_ACTIONS = [
  "notify",
  "reassign",
  "auto_approve",
  "auto_reject",
...
assert(
  matchesCondition(
    { field: "changeType", operator: "in", value: ["normal", "standard"] },
    entity,
  ) === true,
  "in match",
);
...
assert(
  matchesCondition({ field: "impact", operator: "gte", value: 3 }, entity) ===
    true,
  "gte match",
);
assert(
  matchesCondition({ field: "impact", operator: "lte", value: 3 }, entity) ===
    true,
  "lte match",
);
...
assert(
  canTransition("rejected", "approved") === false,
  "rejected->approved blocked",
);
assert(
  canTransition("expired", "pending") === true,
  "expired->pending (retry)",
);
assert(
  canTransition("cancelled", "pending") === true,
  "cancelled->pending (retry)",
);
assert(
  canTransition("skipped", "pending") === true,
...
assert(
  allowedTransitions("approved").length === 0,
  "approved has 0 transitions",
);

console.log("\n=== STEP STATE MACHINE ===\n");

assert(
  canTransition("pending", "active", STEP_TRANSITIONS) === true,
  "step pending->active",
);
assert(
  canTransition("pending", "skipped", STEP_TRANSITIONS) === true,
  "step pending->skipped",
);
assert(
  canTransition("active", "approved", STEP_TRANSITIONS) === true,
  "step active->approved",
);
assert(
  canTransition("active", "rejected", STEP_TRANSITIONS) === true,
  "step active->rejected",
);
assert(
  canTransition("active", "expired", STEP_TRANSITIONS) === true,
  "step active->expired",
);
assert(
  canTransition("active", "delegated", STEP_TRANSITIONS) === true,
  "step active->delegated",
);
assert(
  canTransition("approved", "active", STEP_TRANSITIONS) === false,
  "step approved->active blocked",
);
assert(
  canTransition("expired", "active", STEP_TRANSITIONS) === true,
  "step expired->active (retry)",
);
assert(
  canTransition("delegated", "active", STEP_TRANSITIONS) === true,
  "step delegated->active (retry)",
);
...
assert(
  validateDelegation(badDates).length > 0,
...
assert(
  resolvedNoDel[0] === "agent2",
  "agent step without delegation uses original",
);
...
assert(
  recomputeInstanceStatus([{ status: "approved" }, { status: "approved" }]) ===
    "approved",
  "all approved = approved",
);
assert(
  recomputeInstanceStatus([{ status: "approved" }, { status: "rejected" }]) ===
    "rejected",
  "1 rejected = rejected",
);
assert(
  recomputeInstanceStatus([{ status: "pending" }, { status: "pending" }]) ===
    "pending",
  "all pending = pending",
);
assert(
  recomputeInstanceStatus([{ status: "approved" }, { status: "skipped" }]) ===
    "approved",
  "approved+skipped = approved",
);
assert(
  recomputeInstanceStatus([{ status: "skipped" }, { status: "skipped" }]) ===
    "skipped",
  "all skipped = skipped",
);
assert(
  recomputeInstanceStatus([{ status: "approved" }, { status: "pending" }]) ===
    "pending",
...
assert(
  isSequentialComplete(steps, 2) === true,
  "step 1 complete -> can start step 2",
);
assert(
  isSequentialComplete(steps, 3) === false,
  "step 2 not complete -> cannot start step 3",
);
steps[1].status = "approved";
assert(
  isSequentialComplete(steps, 3) === true,
  "step 2 complete -> can start step 3",
);
