...
} = require("../src/middleware/fieldRbac");
...
(async () => {
  ...
  assert(
    canTransition("task", "open", "in_progress") === true,
    "SM: open->in_progress",
  );
  assert(
    canTransition("task", "in_progress", "resolved") === true,
    "SM: in_progress->resolved",
  );
  assert(
    canTransition("task", "resolved", "closed") === true,
    "SM: resolved->closed",
  );
  assert(
    canTransition("task", "closed", "open") === true,
    "SM: closed->open (reopen)",
  );
  assert(
    canTransition("task", "cancelled", "open") === true,
    "SM: cancelled->open (reopen)",
  );

  // Invalid transitions
  assert(
    canTransition("task", "new", "closed") === false,
    "SM: new cannot close",
  );
  assert(
    canTransition("task", "new", "resolved") === false,
    "SM: new cannot resolve",
  );
  assert(
    canTransition("task", "open", "closed") === false,
    "SM: open cannot close directly",
  );
  assert(
    canTransition("task", "in_progress", "cancelled") === false,
    "SM: in_progress cannot cancel",
  );
  assert(
    canTransition("task", "resolved", "cancelled") === false,
    "SM: resolved cannot cancel",
  );
  assert(
    canTransition("task", "closed", "in_progress") === false,
    "SM: closed cannot go to in_progress",
  );
  assert(
    canTransition("task", "cancelled", "in_progress") === false,
    "SM: cancelled cannot go to in_progress",
  );

  // Pending transitions
  assert(
    canTransition("task", "open", "pending_customer") === true,
    "SM: open->pending_customer",
  );
  assert(
    canTransition("task", "open", "pending_vendor") === true,
    "SM: open->pending_vendor",
  );
  assert(
    canTransition("task", "open", "pending_approval") === true,
    "SM: open->pending_approval",
  );
  assert(
    canTransition("task", "open", "on_hold") === true,
    "SM: open->on_hold",
  );
  assert(
    canTransition("task", "pending_customer", "in_progress") === true,
    "SM: pending_customer->in_progress",
  );
  assert(
    canTransition("task", "pending_vendor", "in_progress") === true,
    "SM: pending_vendor->in_progress",
  );
  assert(
    canTransition("task", "pending_approval", "in_progress") === true,
    "SM: pending_approval->in_progress",
  );
  assert(
    canTransition("task", "on_hold", "in_progress") === true,
    "SM: on_hold->in_progress",
  );

  // No-op
  assert(
    canTransition("task", "open", "open") === true,
    "SM: no-op transition",
  );
  ...
  assert(
    newTrans.includes("open") &&
      newTrans.includes("in_progress") &&
      newTrans.includes("cancelled"),
    "SM: new allows open/in_progress/cancelled",
  );
  ...
  threw = false;
  try {
    validate(schemas.CREATE_TASK, { title: "" });
  } catch (e) {
    threw = true;
  }
  ...
  threw = false;
  try {
    validate(schemas.CREATE_TASK, { title: "Test", priority: "invalid" });
  } catch (e) {
    threw = true;
  }
  ...
  threw = false;
  try {
    validate(schemas.CREATE_TASK, { title: "Test", type: "invalid" });
  } catch (e) {
    threw = true;
  }
  ...
  threw = false;
  try {
    validate(schemas.TRANSITION_TASK, {});
  } catch (e) {
    threw = true;
  }
  ...
  threw = false;
  try {
    validate(schemas.TRANSITION_TASK, { state: "invalid_state" });
  } catch (e) {
    threw = true;
  }
  ...
  threw = false;
  try {
    validate(schemas.ADD_COMMENT, { content: "" });
  } catch (e) {
    threw = true;
  }
  ...
  validate(schemas.ADD_RELATIONSHIP, {
    targetTaskId: "abc",
    relationshipType: "relates_to",
  }); // should pass
  threw = false;
  try {
    validate(schemas.ADD_RELATIONSHIP, { targetTaskId: "abc" });
  } catch (e) {
    threw = true;
  }
  ...
  threw = false;
  try {
    validate(schemas.DECIDE_APPROVAL, { decision: "invalid" });
  } catch (e) {
    threw = true;
  }
  ...
  const writeFiltered = filterFields(
    { name: "John", phone: "123", email: "new@test.com", notes: "secret" },
    restrictions,
    "write",
  );
  assert(
    writeFiltered.name === "John",
    "RBAC: visible field preserved on write",
  );
  assert(
    writeFiltered.phone === undefined,
    "RBAC: hidden field removed on write",
  );
  assert(
    writeFiltered.email === undefined,
    "RBAC: readonly field removed on write",
  );
  assert(
    writeFiltered.notes === undefined,
    "RBAC: hidden field removed on write",
  );
  ...
  assert(
    noFilter.name === "John" && noFilter.phone === "123",
    "RBAC: empty restrictions pass all",
  );

  // Array filtering
  const arrFiltered = filterFields(
    [
      { phone: "1", name: "A" },
      { phone: "2", name: "B" },
    ],
  ...
  assert(
    arrFiltered[0].phone === undefined && arrFiltered[0].name === "A",
    "RBAC: array items filtered",
  );
  assert(
    arrFiltered[1].phone === undefined && arrFiltered[1].name === "B",
    "RBAC: array items filtered consistently",
  );

  // applyFieldRestrictionsToResponse
  const fullData = { name: "John", phone: "123", email: "j@test.com" };
  const restrictedData = applyFieldRestrictionsToResponse(fullData, {
    phone: "hide",
  });
  assert(
    restrictedData.phone === undefined,
    "RBAC: applyFieldRestrictionsToResponse hides",
  );
  assert(
    restrictedData.name === "John",
    "RBAC: applyFieldRestrictionsToResponse preserves",
  );

  // No restrictions returns data as-is
  const unfiltered = applyFieldRestrictionsToResponse(fullData, null);
  assert(
    unfiltered.phone === "123",
    "RBAC: null restrictions return data as-is",
  );
  ...
  assert(
    ...
    "CROSS_TENANT: different tenant queries",
  );
  ...
  threw = false;
  try {
    if (String(taskId) === String(taskId))
      throw new Error("Self-link prevented");
  } catch (e) {
    threw = true;
  }
  ...
  assert(
    ...
    "CROSS_TENANT: tenant IDs differ",
  );
  ...
  assert(
    auditEvent.changedFields.length === 2,
    "AUDIT: changed fields tracked",
  );
  assert(
    auditEvent.correlationId === "corr_123",
    "AUDIT: correlation ID propagated",
  );
  ...
  assert(
    denialEvent.denialReason === "Insufficient permissions",
    "AUDIT: denial reason recorded",
  );
