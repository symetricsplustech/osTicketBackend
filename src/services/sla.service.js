  ...
  await SlaEvent.create({
    ...
    dueAt:
      clock === "first_response"
        ...
        : ticket.resolutionDueAt || ticket.dueDate,
    ...extra,
  }).catch(() => {});
};
...
const dueInHours = async (plan, hours, startDate, company) => {
  ...
  while (remainingMinutes > 0 && steps < maxSteps) {
    ...
    steps += 1;
    if (
      isWithinPlanHours(due, plan) &&
      !isHolidayInTimezone(due, plan.timezone, holidays)
    ) {
      remainingMinutes -= 1;
    }
  }
  if (remainingMinutes > 0)
    throw new Error(
      "SLA target exceeds the supported five-year schedule window",
    );
  return due;
};
...
const computeDueDate = async (sla, startDate = new Date(), opts = {}) => {
  ...
  return dueInHours(
    ...
    hours,
    startDate,
    plan.company || opts.company || null,
  );
};
...
const startClocks = async (sla, startDate = new Date(), opts = {}) => {
  ...
  const responseHours =
    plan.targets && typeof plan.targets.first_response === "number"
      ? plan.targets.first_response
      : plan.gracePeriod || 24;
  const resolutionHours =
    plan.targets && typeof plan.targets.resolution === "number"
...
const pauseSla = async (ticket, waitingOn) => {
  ...
  await recordSlaEvent(ticket, "paused", "resolution", {
    reason: ticket.waitingOn,
  });
  emit("sla.paused", {
    company: ticket.company,
    ticketId: ticket._id,
...
const resumeSla = async (ticket) => {
  ...
  const pausedMs = ticket.slaPausedAt
    ? Date.now() - new Date(ticket.slaPausedAt).getTime()
    : 0;
  if (pausedMs > 0 && ticket.dueDate) {
    ...
    if (!ticket.responseMetAt && ticket.responseDueAt)
      ticket.responseDueAt = new Date(
        new Date(ticket.responseDueAt).getTime() + pausedMs,
      );
  }
  ...
  await ticket.save().catch(() => {});
  await recordSlaEvent(ticket, "resumed", "resolution", {
...
const predictBreachRisk = async (ticket) => {
  ...
  const total = ticket.slaStartedAt
    ? now -
      new Date(ticket.slaStartedAt).getTime() -
      (ticket.slaPausedAt ? now - new Date(ticket.slaPausedAt).getTime() : 0)
    : Math.max(remaining, 1);
  let risk = Math.max(
    0,
    Math.min(100, 100 - (remaining / Math.max(total, 1)) * 100),
  );
  // workload penalty
...
const markOverdueTickets = async () => {
  ...
  for (const t of candidates) {
    ...
    await recordSlaEvent(t, "breached", "resolution", {
      completedAt: now,
      actual:
        new Date(t.dueDate).getTime() -
        new Date(t.slaStartedAt || t.createdAt).getTime(),
      unit: "milliseconds",
    });
    emit("ticket.overdue", {
      company: t.company,
    ...
    if (plan && plan.notifyOnBreach) {
      if (t.agent) {
        ...
        const assigned = t.agent
          ? await require("../models/Agent")
              .findById(t.agent)
              .select("email name")
              .lean()
          : null;
...
const executeEscalationRules = async () => {
  ...
  for (const ticket of open) {
    for (const rule of ticket.sla?.escalationRules || []) {
      ...
      if (
        Date.now() - new Date(anchor).getTime() <
        Number(rule.afterMinutes || 0) * 60000
      )
        continue;
      const exists = await SlaEvent.exists({
        ticket: ticket._id,
        event: "warning",
      ...
      for (const action of rule.actions || []) {
        ...
        if (action.type === "notify_agent" && ticket.agent)
          await require("./notification.service")
            .notifyAgent({
              agentId: ticket.agent,
              type: "sla_breach",
              message: `SLA escalation for #${ticket.number}`,
          ...
          action.type === "notify_company_admin" ||
          action.type === "notify_team_lead" ||
          action.type === "notify_department_manager"
        )
          await require("./notification.service")
            .notifyAdminRoom({
              type: "sla_breach",
          ...
          ticket.priority =
            ticket.priority === "Emergency" ? "Emergency" : "Emergency";
        else if (action.type === "escalate_ticket")
          ticket.status = Ticket.STATUSES.ESCALATED;
        emit(`sla.action.${action.type}`, payload); // email/SMS/push/webhook/major-incident workers consume this
      }
      await ticket.save().catch(() => {});
      await recordSlaEvent(ticket, "warning", rule.clock || "resolution", {
        metadata: { ruleId: String(rule._id), actions: rule.actions },
        reason: `Escalation at ${rule.afterMinutes} minutes`,
      });
      executed += 1;
    }
  }
...
const markResponseBreaches = async () => {
  ...
  for (const t of open) {
    ...
    await recordSlaEvent(t, "breached", "first_response", {
      completedAt: now,
      actual:
        new Date(t.responseDueAt).getTime() -
        new Date(t.slaStartedAt || t.createdAt).getTime(),
      unit: "milliseconds",
    });
    if (t.agent) {
      ...
      const assigned = await require("../models/Agent")
        .findById(t.agent)
        .select("email name")
        .lean();
      if (assigned?.email) {
...
const isWithinPlanHours = (date, plan) => {
  ...
  return (
    minutes >= toMinutes(plan.businessHours.start) &&
    minutes < toMinutes(plan.businessHours.end)
  );
};
