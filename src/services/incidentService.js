...
const { assertTransition, INCIDENT_TRANSITIONS } = require('./stateMachine.service');
...
class IncidentService {
  ...
  static async create(data, actorId) {
    ...
    const priority = data.priority || Incident.calculatePriority(data.impact || '3', data.urgency || '3');
    ...
    events.emit('incident.created', { incidentId: incident._id, tenantId: incident.company });
  ...
  static async list(tenantId, filters = {}) {
    ...
    const [items, total] = await Promise.all([
      Incident.find(query).sort(sort).skip(skip).limit(limit).populate('assignedTo', 'name email').populate('assignmentGroup', 'name'),
  ...
  static async update(incidentId, data, tenantId, actorId) {
    ...
    if (data.assignedTo && String(data.assignedTo) !== String(incident.assignedTo)) {
      await IncidentAssignmentHistory.create({
        incident: incident._id,
        company: tenantId,
    ...
    if (data.assignmentGroup && String(data.assignmentGroup) !== String(incident.assignmentGroup)) {
      await IncidentAssignmentHistory.create({
        incident: incident._id,
        company: tenantId,
        assignmentType: 'reassignment',
  ...
  static async transition(incidentId, targetStatus, tenantId, actorId, notes = '') {
    const incident = await this.getById(incidentId, tenantId);
    ...
    events.emit('status.incident', { incidentId: incident._id, tenantId, from: previousStatus, to: targetStatus });
  ...
  static async assign(incidentId, assignment, tenantId, actorId) {
    ...
    if (assignment.assignmentGroup) incident.assignmentGroup = assignment.assignmentGroup;
  ...
  static async linkCI(incidentId, ciId, role, tenantId, actorId) {
    const existing = await IncidentCI.findOne({ incident: incidentId, ci: ciId });
    if (existing) {
  ...
  static async listCIs(incidentId, tenantId) {
    return IncidentCI.find({ incident: incidentId, company: tenantId, isActive: true }).populate('ci');
  }

  static async linkServiceOffering(incidentId, serviceOfferingId, role, tenantId, actorId) {
    const existing = await IncidentServiceOffering.findOne({ incident: incidentId, serviceOffering: serviceOfferingId });
    if (existing) {
      existing.role = role || existing.role;
      existing.isActive = true;
      await existing.save();
  ...
  static async linkIncident(sourceId, targetId, relationshipType, tenantId, actorId) {
    const existing = await IncidentRelationship.findOne({ sourceIncident: sourceId, targetIncident: targetId });
    if (existing) {
  ...
  static async resolve(incidentId, resolutionData, tenantId, actorId) {
    const incident = await this.transition(incidentId, 'resolved', tenantId, actorId, resolutionData.notes);
  ...
  static async reopen(incidentId, tenantId, actorId, reason = '') {
    ...
    if (['investigating', 'identified', 'monitoring'].includes(incident.status)) {
      targetStatus = 'in_progress';
    }

    return this.transition(incidentId, targetStatus, tenantId, actorId, reason || 'Reopened');
  }
  ...
  static async getAssignmentHistory(incidentId, tenantId) {
    return IncidentAssignmentHistory.find({ incident: incidentId, company: tenantId }).sort({ assignedAt: -1 });
  }
