...
async function effectiveAccess(principal, principalType) {
  ...
  const denied = new Set(
    roles.flatMap((role) =>
      (role.deniedPermissions || []).map((p) => String(p).replace(/^!/, "")),
    ),
  );
  ...
  return {
    ...
    modules: new Set(
      roles
        .flatMap((role) => role.moduleKeys || [])
        .concat(
          assignments.flatMap((assignment) => assignment.moduleKeys || []),
        ),
    ),
    scopes: assignments.map((assignment) => ({
      units: assignment.unitScopes,
      departments: assignment.departmentScopes,
      locations: assignment.locationScopes,
      teams: assignment.teamScopes,
    })),
  };
}
