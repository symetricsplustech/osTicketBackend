...
async function fetchTrainingSet(company) {
  ...
  const docs = await Ticket.find(query)
    .select('number subject title body department dept topic priority tags intent resolution')
...
function score(text, doc) {
  ...
  let matches = 0;
  query.forEach((token) => { if (source.has(token)) matches += 1; });
...
function topVote(docs, field, confidence) {
  ...
  return winner ? { value: winner[0], confidence: Math.min(1, winner[1] / Math.max(1, docs.length)) * confidence } : null;
}
