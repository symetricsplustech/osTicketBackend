...
exports.listSchedules = async (ctx, query = {}) => {
  ...
  const filter = { tenantId, isDeleted: false, ...pick(query, ['status', 'team', 'scheduleType']) };
...
exports.getSchedule = async (ctx, scheduleId) => {
  const tenantId = requireTenant(ctx);
  const sched = await OnCallSchedule.findOne({ _id: scheduleId, tenantId, isDeleted: false });
  if (!sched) throw Object.assign(new Error('On-call schedule not found'), { statusCode: 404 });
  return sched;
};
...
exports.createSchedule = async (ctx, data, actor) => {
  ...
  return OnCallSchedule.create({ ...data, tenantId, number, createdBy: actor.userId });
};
...
exports.updateSchedule = async (ctx, scheduleId, data, actor) => {
  ...
  const sched = await OnCallSchedule.findOne({ _id: scheduleId, tenantId, isDeleted: false });
  if (!sched) throw Object.assign(new Error('On-call schedule not found'), { statusCode: 404 });
  const allowed = ['name', 'description', 'timezone', 'team', 'status', 'scheduleType', 'startDate', 'endDate', 'handoverTime', 'handoverDuration', 'autoNotify', 'notifyBeforeMinutes', 'notifyChannels', 'escalationPolicyId', 'metadata'];
...
exports.deleteSchedule = async (ctx, scheduleId, actor) => {
  ...
  const sched = await OnCallSchedule.findOne({ _id: scheduleId, tenantId, isDeleted: false });
  if (!sched) throw Object.assign(new Error('On-call schedule not found'), { statusCode: 404 });
  sched.isDeleted = true; sched.deletedAt = new Date(); sched.deletedBy = actor.userId;
  await sched.save();
  return { success: true };
};
...
exports.publishSchedule = async (ctx, scheduleId, actor) => {
  ...
  const sched = await OnCallSchedule.findOne({ _id: scheduleId, tenantId, isDeleted: false });
  if (!sched) throw Object.assign(new Error('On-call schedule not found'), { statusCode: 404 });
  if (sched.status === 'active') throw Object.assign(new Error('Schedule already active'), { statusCode: 400 });
  sched.status = 'active';
  await sched.save();
  await auditEventService.log({ tenantId, actorId: actor.userId, action: 'onCallSchedule.publish', entityType: 'OnCallSchedule', entityId: scheduleId });
  return sched;
};
...
exports.listShifts = async (ctx, query = {}) => {
  const tenantId = requireTenant(ctx);
  const filter = { tenantId, isDeleted: false, ...pick(query, ['scheduleId', 'status', 'onCallAgent']) };
  if (query.from) filter.startDate = { $gte: new Date(query.from) };
  if (query.to) filter.endDate = { ...filter.endDate, $lte: new Date(query.to) };
  return Shift.find(filter).sort({ startDate: 1 }).populate('onCallAgent', 'name email');
};

exports.getShift = async (ctx, shiftId) => {
  const tenantId = requireTenant(ctx);
  const shift = await Shift.findOne({ _id: shiftId, tenantId, isDeleted: false }).populate('onCallAgent secondaryAgent', 'name email');
...
exports.updateShift = async (ctx, shiftId, data, actor) => {
  ...
  if (!shift) throw Object.assign(new Error('Shift not found'), { statusCode: 404 });
  const allowed = ['name', 'startDate', 'endDate', 'onCallAgent', 'secondaryAgent', 'status', 'handoverNotes', 'handoverCompleted', 'handoverCompletedAt', 'handoverCompletedBy', 'escalationPolicySnapshot', 'metadata'];
...
exports.deleteShift = async (ctx, shiftId, actor) => {
  ...
  const shift = await Shift.findOne({ _id: shiftId, tenantId, isDeleted: false });
  if (!shift) throw Object.assign(new Error('Shift not found'), { statusCode: 404 });
  shift.isDeleted = true; shift.deletedAt = new Date(); shift.deletedBy = actor.userId;
  await shift.save();
  return { success: true };
};
...
exports.listRosters = async (ctx, query = {}) => {
  const tenantId = requireTenant(ctx);
  const filter = { tenantId, isDeleted: false, ...pick(query, ['scheduleId', 'status']) };
  return Roster.find(filter).sort({ startDate: 1 });
};

exports.getRoster = async (ctx, rosterId) => {
  const tenantId = requireTenant(ctx);
  const roster = await Roster.findOne({ _id: rosterId, tenantId, isDeleted: false });
  if (!roster) throw Object.assign(new Error('Roster not found'), { statusCode: 404 });
  return roster;
};
...
exports.updateRoster = async (ctx, rosterId, data, actor) => {
  ...
  const roster = await Roster.findOne({ _id: rosterId, tenantId, isDeleted: false });
  if (!roster) throw Object.assign(new Error('Roster not found'), { statusCode: 404 });
  const allowed = ['name', 'description', 'startDate', 'endDate', 'status', 'rotation', 'assignedMembers', 'minCoverage', 'maxCoverage', 'handoverWindow', 'metadata'];
...
exports.deleteRoster = async (ctx, rosterId, actor) => {
  ...
  const roster = await Roster.findOne({ _id: rosterId, tenantId, isDeleted: false });
  if (!roster) throw Object.assign(new Error('Roster not found'), { statusCode: 404 });
  roster.isDeleted = true; roster.deletedAt = new Date(); roster.deletedBy = actor.userId;
  await roster.save();
  return { success: true };
};
...
exports.listRosterMembers = async (ctx, rosterId) => {
  const tenantId = requireTenant(ctx);
  return RosterMember.find({ tenantId, rosterId, isActive: true }).populate('userId', 'name email');
};

exports.addRosterMember = async (ctx, rosterId, data, actor) => {
  const tenantId = requireTenant(ctx);
  return RosterMember.create({ tenantId, rosterId, ...data, createdBy: actor.userId });
};
...
exports.getRotation = async (ctx, rotationId) => {
  ...
  if (!rot) throw Object.assign(new Error('Rotation not found'), { statusCode: 404 });
  return rot;
};
...
exports.updateRotation = async (ctx, rotationId, data) => {
  ...
  const rot = await Rotation.findOne({ _id: rotationId, tenantId });
  if (!rot) throw Object.assign(new Error('Rotation not found'), { statusCode: 404 });
  const allowed = ['name', 'type', 'pattern', 'members', 'currentIndex', 'nextHandoverDate', 'status', 'metadata'];
...
exports.advanceRotation = async (ctx, rotationId) => {
  ...
  rot.nextHandoverDate = addDays(rot.nextHandoverDate, rot.pattern.durationDays);
  await rot.save();
  return rot;
};
...
exports.listCoverageRequests = async (ctx, query = {}) => {
  const tenantId = requireTenant(ctx);
  const filter = { tenantId, ...pick(query, ['scheduleId', 'shiftId', 'requesterId', 'status', 'type']) };
  return CoverageRequest.find(filter).sort({ createdAt: -1 }).populate('requesterId targetUserId', 'name email');
};

exports.getCoverageRequest = async (ctx, requestId) => {
  const tenantId = requireTenant(ctx);
  const cr = await CoverageRequest.findOne({ _id: requestId, tenantId });
  if (!cr) throw Object.assign(new Error('Coverage request not found'), { statusCode: 404 });
  return cr;
};

exports.createCoverageRequest = async (ctx, data, actor) => {
  const tenantId = requireTenant(ctx);
  return CoverageRequest.create({ ...data, tenantId, requesterId: actor.userId, createdBy: actor.userId });
};
...
exports.listTimeOffRequests = async (ctx, query = {}) => {
  const tenantId = requireTenant(ctx);
  const filter = { tenantId, ...pick(query, ['scheduleId', 'userId', 'status']) };
  return TimeOffRequest.find(filter).sort({ createdAt: -1 }).populate('userId', 'name email');
};

exports.getTimeOffRequest = async (ctx, requestId) => {
  const tenantId = requireTenant(ctx);
  const tor = await TimeOffRequest.findOne({ _id: requestId, tenantId });
  if (!tor) throw Object.assign(new Error('Time-off request not found'), { statusCode: 404 });
  return tor;
};

exports.createTimeOffRequest = async (ctx, data, actor) => {
  const tenantId = requireTenant(ctx);
  return TimeOffRequest.create({ ...data, tenantId, userId: actor.userId, createdBy: actor.userId });
};
...
exports.listEscalationPolicies = async (ctx, query = {}) => {
  const tenantId = requireTenant(ctx);
  const filter = { tenantId, isDeleted: false, ...pick(query, ['isActive', 'team', 'isDefault']) };
  return EscalationPolicy.find(filter).sort({ name: 1 });
};

exports.getEscalationPolicy = async (ctx, policyId) => {
  const tenantId = requireTenant(ctx);
  const policy = await EscalationPolicy.findOne({ _id: policyId, tenantId, isDeleted: false });
  if (!policy) throw Object.assign(new Error('Escalation policy not found'), { statusCode: 404 });
  return policy;
};
...
exports.createEscalationPolicy = async (ctx, data, actor) => {
  ...
  return EscalationPolicy.create({ ...data, tenantId, number, createdBy: actor.userId });
};
...
exports.updateEscalationPolicy = async (ctx, policyId, data) => {
  ...
  const policy = await EscalationPolicy.findOne({ _id: policyId, tenantId, isDeleted: false });
  if (!policy) throw Object.assign(new Error('Escalation policy not found'), { statusCode: 404 });
  const allowed = ['name', 'description', 'isActive', 'isDefault', 'team', 'repeatAfterMinutes', 'maxRepeats', 'onCallOnly', 'metadata'];
...
exports.deleteEscalationPolicy = async (ctx, policyId, actor) => {
  ...
  const policy = await EscalationPolicy.findOne({ _id: policyId, tenantId, isDeleted: false });
  if (!policy) throw Object.assign(new Error('Escalation policy not found'), { statusCode: 404 });
  policy.isDeleted = true; policy.deletedAt = new Date(); policy.deletedBy = actor.userId;
  await policy.save();
  return { success: true };
};
...
exports.updateEscalationLevel = async (ctx, levelId, data) => {
  ...
  if (!level) throw Object.assign(new Error('Escalation level not found'), { statusCode: 404 });
  const allowed = ['name', 'delayMinutes', 'targets', 'notifyUntilAcknowledged', 'acknowledgementTimeoutMinutes', 'autoEscalateIfNoAck', 'metadata'];
...
exports.updateContactPreference = async (ctx, userId, data) => {
  ...
  let pref = await ContactPreference.findOne({ tenantId, userId });
  if (!pref) pref = await ContactPreference.create({ tenantId, userId });
  const allowed = ['onCallEmail', 'onCallSms', 'onCallVoice', 'onCallSlack', 'onCallTeams', 'preferredOrder', 'quietHours', 'escalationMode', 'acknowledgeTimeoutMinutes', 'metadata'];
...
exports.getCurrentOnCall = async (ctx, scheduleId) => {
  ...
  const shift = await Shift.findOne({
    tenantId, scheduleId, isDeleted: false,
    startDate: { $lte: now },
    endDate: { $gte: now },
...
exports.getUpcomingShifts = async (ctx, scheduleId, days = 7) => {
  ...
  return Shift.find({
    ...
    status: { $in: ['scheduled', 'active'] },
  }).sort({ startDate: 1 }).populate('onCallAgent secondaryAgent', 'name email');
};

exports.getOnCallAgentForTeam = async (ctx, teamId) => {
  const tenantId = requireTenant(ctx);
  const schedule = await OnCallSchedule.findOne({ tenantId, team: teamId, status: 'active', isDeleted: false });
...
exports.detectGaps = async (ctx, scheduleId, days = 14) => {
  ...
  for (const shift of shifts) {
    if (shift.startDate > prevEnd) {
      gaps.push({ from: prevEnd, to: shift.startDate, durationMinutes: (shift.startDate - prevEnd) / 60000 });
    }
...
exports.getDashboard = async (ctx) => {
  const tenantId = requireTenant(ctx);
  const [schedules, shifts, rosters, coverage, timeOff, policies, contacts] = await Promise.all([
    OnCallSchedule.find({ tenantId, isDeleted: false }),
    Shift.find({ tenantId, isDeleted: false, startDate: { $gte: new Date() } }),
  ...
  const upcomingShifts = shifts.filter(s => s.status === 'scheduled' || s.status === 'active').length;
  ...
  for (const shift of shifts) {
    ...
    for (const agent of agents) {
      ...
      emitEvent(tenantId, 'oncall.shift.upcoming', {
        shiftId: shift._id, scheduleId: shift.scheduleId,
        startDate: shift.startDate, endDate: shift.endDate,
