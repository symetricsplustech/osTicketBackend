...
const safeFilename = (name) =>
  (name || "attachment").replace(/[^a-zA-Z0-9._-]/g, "_");
...
const resolveInboundCompany = async (opts) => {
  ...
  if (process.env.EMAIL_DEFAULT_COMPANY)
    return process.env.EMAIL_DEFAULT_COMPANY || null;
  ...
  if (to.length) {
    ...
    const domains = [
      ...new Set(to.map((a) => a.split("@")[1]).filter(Boolean)),
    ];
    if (domains.length) {
  ...
  const company = await Company.findOne({ status: "active" }).sort({
    createdAt: 1,
  });
...
const isBannedSender = async (email) => {
  ...
  const banList = (settings.emails?.banList || []).map((b) =>
...
const findHelpTopic = async (companyId) => {
  ...
  const generalTopic = await HelpTopic.findOne({
    ...base,
...
const notifyAgentsForReply = async (ticket, sender) => {
  ...
  const deptAgents = await Agent.find({
    "departments.department": ticket.dept,
    isActive: true,
...
const handleNewTicket = async ({
  ...
  const sender =
    existing ||
    (await ticketService.findOrCreateUser({
      name: senderName,
      email: senderEmail,
      company: companyId,
    }));
...
const findTicketByHeaders = async (inReplyTo = [], references = []) => {
  ...
  const outbound = await EmailLog.findOne({
    ...query,
    event: { $ne: "inbound_processed" },
  })
    .sort({ createdAt: -1 })
...
const isAuthorizedReplySender = async (ticket, senderEmail) => {
  ...
  if (ticket.collaborators?.length) {
    const collabs = await User.find({ _id: { $in: ticket.collaborators } })
      .select("email")
      .lean();
...
const processParsedEmail = async ({
  ...
  if (await isSystemSent(messageId))
    return { action: "skipped", reason: "system generated" };
  if (await isBannedSender(senderEmail))
    return { action: "skipped", reason: "banned sender" };
  if (
    BOUNCE_SENDERS.has(String(senderEmail).toLowerCase()) ||
  ...
  if (ticket) {
    ...
    const authorized = await isAuthorizedReplySender(ticket, senderEmail).catch(
      () => false,
    );
    if (!authorized) {
      logger.warn(
        `Unauthorized email reply from ${senderEmail} for ticket ${ticket.number}; creating new ticket`,
      );
      ticket = null;
      matchedVia = null;
    }
  ...
  if (ticket) {
    ...
    const existingSender = await User.findOne({
      email: normalizeAddr(senderEmail),
    });
    const sender =
      existingSender ||
      (await ticketService.findOrCreateUser({
    ...
    if (
      ...
      if (
        replySettings &&
        !ticketService.reopenWindowAllows(ticket, replySettings)
      ) {
...
const parseMessage = async (message) => {
  ...
  const toAddresses = [...collect(parsed.to), ...collect(parsed.cc)]
  ...
  return {
    ...
    senderName:
      from.name || parsed.from?.text?.split("<")[0]?.trim() || "Email Sender",
...
const pollInbox = async ({ sinceDays } = {}) => {
  ...
  const since = new Date(
    Date.now() -
      (sinceDays || config.email.imapWindowDays) * 24 * 60 * 60 * 1000,
  );
  const summary = {
    processed: 0,
    newTickets: 0,
    ...
    try {
      ...
      for (const uid of search) {
        try {
          ...
          logger.error(
            `Inbound email processing failed for UID ${uid}: ${err.message}`,
          );
        }
      }
    } finally {
...
const startInboundPoller = () => {
  if (!config.email.imapEnabled) {
    logger.info(
      "Inbound email polling disabled (set EMAIL_IMAP_ENABLED=false to keep off).",
    );
    return;
  }
  logger.info(
    `Inbound email polling started for ${config.email.imapUser} (${config.email.imapMailbox}) every ${config.email.imapPollInterval / 1000}s`,
  );
  setInterval(() => {
    pollInbox()
      .then((s) => {
        if (s.processed > 0)
          logger.info(`Inbound poll summary: ${JSON.stringify(s)}`);
      })
