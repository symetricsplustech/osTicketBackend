...
const scoreAgent = (
  ...
  if (customerTier === "enterprise" || customerTier === "priority")
    score += (3 - level) * 3;
...
async function findBestAgent(opts) {
  ...
  if (algorithm === "round_robin") {
    ...
    const candidates = pool.filter(
      (a) => a.presence !== "offline" && a.presence !== "dnd",
    );
  ...
  if (algorithm === "least_workload") {
    pool.sort(
      (a, b) =>
        a._active - b._active ||
        PRESENCE_WEIGHT[a.presence] - PRESENCE_WEIGHT[b.presence],
    );
  ...
  if (algorithm === "availability") {
    const avail = pool.filter(
      (a) => a.presence === "online" || a.presence === "busy",
    );
