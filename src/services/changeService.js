...
class ChangeService {
  ...
  static async create(data, actorId) {
    ...
    await auditEventService.record({
      ...
      after: change.toObject(),
    });
    events.emit("change.created", {
      changeId: change._id,
      tenantId: change.company,
    });
    return change;
  ...
  static async update(changeId, data, tenantId, actorId) {
    ...
    if (data.status && data.status !== change.status)
      assertTransition("change", change.status, data.status);
  ...
  static async unlinkCI(changeId, ciId, tenantId) {
    return ChangeCI.findOneAndUpdate(
      { change: changeId, ci: ciId, company: tenantId },
      { isActive: false },
      { new: true },
    );
  }
  ...
  static async linkService(
    ...
    const existing = await ChangeServiceLink.findOne({
      change: changeId,
      service: serviceId,
    });
    if (existing) {
      existing.impactLevel = impactLevel || existing.impactLevel;
      existing.isActive = true;
      await existing.save();
      return existing;
    }
    return ChangeServiceLink.create({
      change: changeId,
      service: serviceId,
      company: tenantId,
  ...
  static async assessRisk(changeId, riskData, tenantId, actorId) {
    const existing = await ChangeRiskAssessment.findOne({
      change: changeId,
      company: tenantId,
    });
    if (existing) {
      Object.assign(existing, riskData, {
        assessedBy: actorId,
        assessedAt: new Date(),
      });
      await existing.save();
    ...
    return ChangeRiskAssessment.create({
      ...riskData,
      change: changeId,
      company: tenantId,
      assessedBy: actorId,
    ...
    for (const bw of blackouts) {
      conflicts.push(
        await ChangeConflict.create({
          ...
          company: tenantId,
          conflictType: "blackout_violation",
          description: `Blackout window: ${bw.name}`,
        }),
      );
    }
    return conflicts;
  ...
  static async createTask(changeId, taskData, tenantId, actorId) {
    ...
    return ChangeTask.create({
      ...taskData,
      number: `CHGT-${new Date().getFullYear()}-${String(count + 1).padStart(4, "0")}`,
      change: changeId,
      company: tenantId,
      createdBy: actorId,
    });
  ...
  static async createImplementationResult(changeId, data, tenantId, actorId) {
    const existing = await ChangeImplementationResult.findOne({
      change: changeId,
      company: tenantId,
    });
    if (existing) {
      Object.assign(existing, data, { implementedBy: actorId });
      await existing.save();
      return existing;
    }
    return ChangeImplementationResult.create({
      ...data,
      change: changeId,
      company: tenantId,
      implementedBy: actorId,
    });
  }
  ...
  static async listMaintenanceWindows(tenantId) {
    return MaintenanceWindow.find({ company: tenantId, isActive: true }).sort({
      startTime: 1,
    });
  }

  static async listBlackoutWindows(tenantId) {
    return BlackoutWindow.find({ company: tenantId, isActive: true }).sort({
      startTime: 1,
    });
  }
  ...
  static async listAgendaItems(meetingId, tenantId) {
    return CABAgendaItem.find({ meeting: meetingId, company: tenantId })
      .sort({ order: 1 })
      .populate("change", "number title risk status");
  }

  static async decideAgendaItem(
    itemId,
    decision,
    decisionNotes,
    tenantId,
    ...
    const item = await CABAgendaItem.findOne({
      _id: itemId,
      company: tenantId,
    });
  ...
  static async addAttendee(meetingId, userId, tenantId) {
    const existing = await CABAttendee.findOne({
      meeting: meetingId,
      user: userId,
    });
    if (existing) return existing;
    return CABAttendee.create({
      meeting: meetingId,
      user: userId,
      company: tenantId,
    });
  ...
  static async listAttendees(meetingId, tenantId) {
    return CABAttendee.find({ meeting: meetingId, company: tenantId }).populate(
      "user",
      "name email",
    );
  }
