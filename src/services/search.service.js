...
async function globalSearch({ company, q = "", type, page = 1, limit = 20 }) {
  ...
  if (filters.slaHours) {
    ...
    ticketMatch.dueDate =
      filters.slaHours.op === "<" ? { $lte: hoursAgo } : { $gte: hoursAgo };
  }
  if (filters.customer_tier) {
    const users = await User.find({
      ...comp,
      tier: filters.customer_tier.value,
    }).distinct("_id");
  ...
  if (filters.from) {
    const user = await User.findOne({
      ...comp,
      email: filters.from.value.toLowerCase(),
    }).lean();
  ...
  for (const section of sections) {
    switch (section) {
      ...
      case "organizations": {
        const match = { ...comp };
        if (regex)
          match.$or = [{ name: regex }, { domain: regex }, { website: regex }];
        const [items, total] = await Promise.all([
          Organization.find(match)
            .sort({ name: 1 })
            .skip(skip)
            .limit(sectionLimit)
            .lean(),
      ...
      case "invoices": {
        const match = { ...comp };
        if (regex)
          match.$or = [{ invoiceNumber: regex }, { description: regex }];
        const [items, total] = await Promise.all([
          Invoice.find(match)
            .sort({ createdAt: -1 })
            .skip(skip)
