...
exports.listLocations = async (ctx, query = {}) => {
  ...
  const filter = { tenantId, isDeleted: false, ...pick(query, ['status', 'team']) };
...
exports.getLocation = async (ctx, locationId) => {
  const tenantId = requireTenant(ctx);
  const loc = await WalkupLocation.findOne({ _id: locationId, tenantId, isDeleted: false });
  if (!loc) throw Object.assign(new Error('Walk-up location not found'), { statusCode: 404 });
  return loc;
};
...
exports.createLocation = async (ctx, data, actor) => {
  ...
  return WalkupLocation.create({ ...data, tenantId, number, createdBy: actor.userId });
};
...
exports.updateLocation = async (ctx, locationId, data, actor) => {
  ...
  const loc = await WalkupLocation.findOne({ _id: locationId, tenantId, isDeleted: false });
  if (!loc) throw Object.assign(new Error('Walk-up location not found'), { statusCode: 404 });
  const allowed = ['name', 'description', 'address', 'floor', 'room', 'timezone', 'hours', 'exceptions', 'services', 'kiosks', 'team', 'maxConcurrentCheckins', 'waitTimeEstimation', 'autoAssign', 'checkinMethods', 'status', 'metadata'];
...
exports.deleteLocation = async (ctx, locationId, actor) => {
  ...
  const loc = await WalkupLocation.findOne({ _id: locationId, tenantId, isDeleted: false });
  if (!loc) throw Object.assign(new Error('Walk-up location not found'), { statusCode: 404 });
  loc.isDeleted = true; loc.deletedAt = new Date(); loc.deletedBy = actor.userId;
  await loc.save();
  return { success: true };
};
...
exports.getOpenLocations = async (ctx) => {
  ...
  const locations = await WalkupLocation.find({ tenantId, status: 'active', isDeleted: false });
...
exports.listServices = async (ctx, query = {}) => {
  const tenantId = requireTenant(ctx);
  const filter = { tenantId, isDeleted: false, ...pick(query, ['status', 'category', 'locationId']) };
  if (query.search) filter.$or = [{ name: { $regex: query.search, $options: 'i' } }];
...
exports.getService = async (ctx, serviceId) => {
  const tenantId = requireTenant(ctx);
  const svc = await WalkupService.findOne({ _id: serviceId, tenantId, isDeleted: false });
  if (!svc) throw Object.assign(new Error('Walk-up service not found'), { statusCode: 404 });
  return svc;
};
...
exports.createService = async (ctx, data, actor) => {
  ...
  return WalkupService.create({ ...data, tenantId, number, createdBy: actor.userId });
};
...
exports.updateService = async (ctx, serviceId, data, actor) => {
  ...
  const svc = await WalkupService.findOne({ _id: serviceId, tenantId, isDeleted: false });
  if (!svc) throw Object.assign(new Error('Walk-up service not found'), { statusCode: 404 });
  const allowed = ['name', 'description', 'category', 'locations', 'estimatedDuration', 'requiresAppointment', 'allowsWalkin', 'requiresApproval', 'requiresAsset', 'skillRequired', 'slaTargetMinutes', 'autoCreateTicket', 'ticketTemplate', 'status', 'metadata'];
...
exports.deleteService = async (ctx, serviceId, actor) => {
  ...
  const svc = await WalkupService.findOne({ _id: serviceId, tenantId, isDeleted: false });
  if (!svc) throw Object.assign(new Error('Walk-up service not found'), { statusCode: 404 });
  svc.isDeleted = true; svc.deletedAt = new Date(); svc.deletedBy = actor.userId;
  await svc.save();
  return { success: true };
};
...
exports.listQueues = async (ctx, query = {}) => {
  const tenantId = requireTenant(ctx);
  const filter = { tenantId, isDeleted: false, ...pick(query, ['locationId', 'serviceId', 'status']) };
  return WalkupQueue.find(filter).sort({ name: 1 });
};

exports.getQueue = async (ctx, queueId) => {
  const tenantId = requireTenant(ctx);
  const queue = await WalkupQueue.findOne({ _id: queueId, tenantId, isDeleted: false });
...
exports.createQueue = async (ctx, data, actor) => {
  ...
  return WalkupQueue.create({ ...data, tenantId, number, createdBy: actor.userId });
};
...
exports.updateQueue = async (ctx, queueId, data, actor) => {
  ...
  const queue = await WalkupQueue.findOne({ _id: queueId, tenantId, isDeleted: false });
  if (!queue) throw Object.assign(new Error('Queue not found'), { statusCode: 404 });
  const allowed = ['name', 'serviceId', 'status', 'maxSize', 'priorityHandling', 'autoEstimateWait', 'metadata'];
...
exports.updateQueueStats = async (queueId, tenantId) => {
  const queue = await WalkupQueue.findOne({ _id: queueId, tenantId, isDeleted: false });
  if (!queue) return;
  const waiting = await WalkupCheckin.countDocuments({ tenantId, queueId, status: 'waiting' });
  queue.currentSize = waiting;
  if (queue.autoEstimateWait) {
    const recent = await WaitTimeEvent.find({ tenantId, queueId }).sort({ timestamp: -1 }).limit(10);
    if (recent.length) {
      queue.avgWaitMinutes = Math.round(recent.reduce((s, e) => s + e.waitTimeMinutes, 0) / recent.length);
...
exports.checkIn = async (ctx, data, actor) => {
  ...
  // Validate queue
  const queue = await WalkupQueue.findOne({ _id: data.queueId, tenantId, isDeleted: false });
  if (!queue) throw Object.assign(new Error('Queue not found'), { statusCode: 404 });
  if (queue.status !== 'open') throw Object.assign(new Error('Queue is not open'), { statusCode: 400 });
  if (queue.currentSize >= queue.maxSize) throw Object.assign(new Error('Queue is full'), { statusCode: 400 });
  ...
  emitEvent(tenantId, 'walkup.checkin', { checkinId: checkin._id, queueNumber, queueId: queue._id });
  return checkin;
};
...
exports.callCheckin = async (ctx, checkinId, technicianId, technicianName) => {
  ...
  if (!checkin) throw Object.assign(new Error('Check-in not found'), { statusCode: 404 });
  if (checkin.status !== 'waiting') throw Object.assign(new Error('Check-in not in waiting status'), { statusCode: 400 });
  ...
  await WaitTimeEvent.create({
    ...
    checkinId: checkin._id,
    queueSize: await WalkupCheckin.countDocuments({ tenantId, queueId: checkin.queueId, status: 'waiting' }),
    waitTimeMinutes: checkin.waitMinutes,
    eventType: 'called',
  });

  emitEvent(tenantId, 'walkup.called', { checkinId, queueNumber: checkin.queueNumber, technicianName });
  return checkin;
};
...
exports.startService = async (ctx, checkinId, technicianId, technicianName) => {
  ...
  const checkin = await WalkupCheckin.findOne({ _id: checkinId, tenantId });
  if (!checkin) throw Object.assign(new Error('Check-in not found'), { statusCode: 404 });
  if (checkin.status !== 'called' && checkin.status !== 'waiting') throw Object.assign(new Error('Check-in not in callable status'), { statusCode: 400 });
...
exports.completeCheckin = async (ctx, checkinId, data) => {
  ...
  const checkin = await WalkupCheckin.findOne({ _id: checkinId, tenantId });
  if (!checkin) throw Object.assign(new Error('Check-in not found'), { statusCode: 404 });
  if (checkin.status !== 'in_service') throw Object.assign(new Error('Check-in not in service'), { statusCode: 400 });

  checkin.status = 'completed';
  checkin.completedAt = new Date();
  if (checkin.startedAt) {
    checkin.serviceMinutes = Math.round((new Date() - checkin.startedAt) / 60000);
  }
  ...
  emitEvent(tenantId, 'walkup.completed', { checkinId, outcome: checkin.outcome });
  return checkin;
};
...
exports.cancelCheckin = async (ctx, checkinId, actor) => {
  ...
  const checkin = await WalkupCheckin.findOne({ _id: checkinId, tenantId });
  if (!checkin) throw Object.assign(new Error('Check-in not found'), { statusCode: 404 });
  if (['completed', 'cancelled', 'no_show'].includes(checkin.status)) throw Object.assign(new Error('Check-in already finalized'), { statusCode: 400 });
...
exports.listAppointments = async (ctx, query = {}) => {
  const tenantId = requireTenant(ctx);
  const filter = { tenantId, isDeleted: false, ...pick(query, ['locationId', 'serviceId', 'status', 'userId']) };
...
exports.getAppointment = async (ctx, appointmentId) => {
  const tenantId = requireTenant(ctx);
  const appt = await Appointment.findOne({ _id: appointmentId, tenantId, isDeleted: false });
...
exports.createAppointment = async (ctx, data, actor) => {
  ...
  return Appointment.create({ ...data, tenantId, number, createdBy: actor.userId });
};
...
exports.updateAppointment = async (ctx, appointmentId, data) => {
  ...
  const appt = await Appointment.findOne({ _id: appointmentId, tenantId, isDeleted: false });
  if (!appt) throw Object.assign(new Error('Appointment not found'), { statusCode: 404 });
  const allowed = ['locationId', 'serviceId', 'scheduledAt', 'durationMinutes', 'status', 'checkinMethod', 'reminderSent', 'checkinAt', 'startedAt', 'completedAt', 'technicianId', 'technicianName', 'reason', 'notes', 'outcome', 'ticketId', 'incidentId', 'requestId', 'metadata'];
...
exports.checkinAppointment = async (ctx, appointmentId, data, actor) => {
  ...
  const appt = await Appointment.findOne({ _id: appointmentId, tenantId, isDeleted: false });
  if (!appt) throw Object.assign(new Error('Appointment not found'), { statusCode: 404 });
  if (!['scheduled', 'confirmed'].includes(appt.status)) throw Object.assign(new Error('Appointment not in check-inable status'), { statusCode: 400 });
  ...
  const checkin = await exports.checkIn(ctx, {
...
exports.createInteraction = async (ctx, data, actor) => {
  const tenantId = requireTenant(ctx);
  return WalkupInteraction.create({ ...data, tenantId, createdBy: actor.userId });
};

exports.getInteraction = async (ctx, interactionId) => {
  const tenantId = requireTenant(ctx);
  const interaction = await WalkupInteraction.findOne({ _id: interactionId, tenantId });
  if (!interaction) throw Object.assign(new Error('Interaction not found'), { statusCode: 404 });
  return interaction;
};
...
exports.completeInteraction = async (ctx, interactionId, data) => {
  ...
  interaction.durationMinutes = Math.round((new Date() - interaction.startedAt) / 60000);
  ...
  emitEvent(tenantId, 'walkup.interaction.completed', { interactionId, outcome: interaction.outcome });
  return interaction;
};
...
exports.listKiosks = async (ctx, query = {}) => {
  const tenantId = requireTenant(ctx);
  const filter = { tenantId, isDeleted: false, ...pick(query, ['locationId', 'status']) };
  return Kiosk.find(filter).sort({ name: 1 });
};

exports.getKiosk = async (ctx, kioskId) => {
  const tenantId = requireTenant(ctx);
  const kiosk = await Kiosk.findOne({ _id: kioskId, tenantId, isDeleted: false });
...
exports.updateKiosk = async (ctx, kioskId, data) => {
  ...
  const kiosk = await Kiosk.findOne({ _id: kioskId, tenantId, isDeleted: false });
  if (!kiosk) throw Object.assign(new Error('Kiosk not found'), { statusCode: 404 });
  const allowed = ['name', 'description', 'serialNumber', 'ipAddress', 'macAddress', 'status', 'hardwareInfo', 'softwareVersion', 'configuredServices', 'uiTheme', 'printerEnabled', 'printerConfig', 'scannerEnabled', 'cameraEnabled', 'cardReaderEnabled', 'lastConfigUpdate', 'metadata'];
...
exports.deleteKiosk = async (ctx, kioskId, actor) => {
  ...
  const kiosk = await Kiosk.findOne({ _id: kioskId, tenantId, isDeleted: false });
  if (!kiosk) throw Object.assign(new Error('Kiosk not found'), { statusCode: 404 });
  kiosk.isDeleted = true; kiosk.deletedAt = new Date(); kiosk.deletedBy = actor.userId;
  await kiosk.save();
  return { success: true };
};
...
exports.getWaitTimeEstimate = async (ctx, queueId) => {
  ...
  const queue = await WalkupQueue.findOne({ _id: queueId, tenantId, isDeleted: false });
  if (!queue) throw Object.assign(new Error('Queue not found'), { statusCode: 404 });
  return { estimatedWaitMinutes: queue.estimatedWaitMinutes, avgWaitMinutes: queue.avgWaitMinutes, queueSize: queue.currentSize };
};
...
exports.getDashboard = async (ctx) => {
  const tenantId = requireTenant(ctx);
  const [locations, services, queues, checkins, appointments, kiosks, interactions] = await Promise.all([
    ...
    WalkupCheckin.find({ tenantId, status: { $in: ['waiting', 'called', 'in_service'] } }),
    Appointment.find({ tenantId, isDeleted: false, status: { $in: ['scheduled', 'confirmed', 'checked_in'] } }),
  ...
  const inServiceCount = checkins.filter(c => c.status === 'in_service').length;
  const upcomingAppointments = appointments.filter(a => new Date(a.scheduledAt) >= new Date()).length;
  const onlineKiosks = kiosks.filter(k => k.status === 'online').length;

  const avgWait = queues.length ? Math.round(queues.reduce((s, q) => s + (q.avgWaitMinutes || 0), 0) / queues.length) : 0;
