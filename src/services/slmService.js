...
exports.getPlan = async (ctx, planId) => {
  ...
  if (!plan) throw Object.assign(new Error('SLA plan not found'), { statusCode: 404 });
  return plan;
};
...
exports.createPlan = async (ctx, data, actor) => {
  ...
  await auditEventService.log({ tenantId, actorId: actor.userId, action: 'slaPlan.create', entityType: 'SlaPlan', entityId: plan._id });
  return plan;
};
...
exports.updatePlan = async (ctx, planId, data, actor) => {
  ...
  const plan = await SlaPlan.findOne({ _id: planId, company: tenantId });
  if (!plan) throw Object.assign(new Error('SLA plan not found'), { statusCode: 404 });
  const allowed = ['name', 'gracePeriod', 'schedule', 'timezone', 'businessHours', 'status', 'notes', 'targets', 'pauseRules', 'escalationRules', 'pauseOnWaiting', 'notifyOnBreach', 'notifyOnAtRisk', 'breachEscalate'];
  const before = JSON.parse(JSON.stringify(plan));
  Object.assign(plan, pick(data, allowed));
  await plan.save();
  await auditEventService.log({ tenantId, actorId: actor.userId, action: 'slaPlan.update', entityType: 'SlaPlan', entityId: planId, before, after: plan });
  return plan;
};
...
exports.deletePlan = async (ctx, planId, actor) => {
  ...
  await auditEventService.log({ tenantId, actorId: actor.userId, action: 'slaPlan.delete', entityType: 'SlaPlan', entityId: planId });
  return { success: true };
};
...
exports.clonePlan = async (ctx, planId, data, actor) => {
  ...
  if (!original) throw Object.assign(new Error('SLA plan not found'), { statusCode: 404 });
...
exports.listOLAs = async (ctx, query = {}) => {
  const tenantId = requireTenant(ctx);
  const filter = { tenantId, isDeleted: false, ...pick(query, ['isActive', 'department', 'group']) };
  return OLA.find(filter).sort({ name: 1 }).populate('slaPlanId', 'name').populate('department', 'name');
};

exports.getOLA = async (ctx, olaId) => {
  const tenantId = requireTenant(ctx);
  const ola = await OLA.findOne({ _id: olaId, tenantId, isDeleted: false }).populate('slaPlanId');
  if (!ola) throw Object.assign(new Error('OLA not found'), { statusCode: 404 });
  return ola;
};
...
exports.createOLA = async (ctx, data, actor) => {
  ...
  const ola = await OLA.create({ ...data, tenantId, number, createdBy: actor.userId });
  return ola;
};
...
exports.updateOLA = async (ctx, olaId, data, actor) => {
  ...
  if (!ola) throw Object.assign(new Error('OLA not found'), { statusCode: 404 });
  const allowed = ['name', 'description', 'slaPlanId', 'department', 'group', 'service', 'targets', 'schedule', 'timezone', 'isActive', 'status'];
...
exports.deleteOLA = async (ctx, olaId, actor) => {
  ...
  const ola = await OLA.findOne({ _id: olaId, tenantId, isDeleted: false });
  if (!ola) throw Object.assign(new Error('OLA not found'), { statusCode: 404 });
  ola.isDeleted = true; ola.deletedAt = new Date(); ola.deletedBy = actor.userId;
  await ola.save();
  return { success: true };
};
...
exports.listUCTargets = async (ctx, query = {}) => {
  const tenantId = requireTenant(ctx);
  const filter = { tenantId, isDeleted: false, ...pick(query, ['contractId', 'status', 'isActive']) };
...
exports.createUCTarget = async (ctx, data, actor) => {
  ...
  return UnderpinningContractTarget.create({ ...data, tenantId, number, createdBy: actor.userId });
};

exports.updateUCTarget = async (ctx, targetId, data, actor) => {
  const tenantId = requireTenant(ctx);
  const target = await UnderpinningContractTarget.findOne({ _id: targetId, tenantId, isDeleted: false });
  if (!target) throw Object.assign(new Error('UC target not found'), { statusCode: 404 });
  const allowed = ['name', 'metric', 'description', 'targetValue', 'targetUnit', 'comparisonOperator', 'measurementWindow', 'isActive'];
...
exports.listSchedules = async (ctx, query = {}) => {
  const tenantId = requireTenant(ctx);
  const filter = { tenantId, isDeleted: false, ...pick(query, ['isActive', 'type']) };
  return BusinessSchedule.find(filter).sort({ name: 1 });
};

exports.getSchedule = async (ctx, scheduleId) => {
  const tenantId = requireTenant(ctx);
  const schedule = await BusinessSchedule.findOne({ _id: scheduleId, tenantId, isDeleted: false });
...
exports.createSchedule = async (ctx, data, actor) => {
  ...
  return BusinessSchedule.create({ ...data, tenantId, number, createdBy: actor.userId });
};
...
exports.updateSchedule = async (ctx, scheduleId, data, actor) => {
  ...
  const schedule = await BusinessSchedule.findOne({ _id: scheduleId, tenantId, isDeleted: false });
  if (!schedule) throw Object.assign(new Error('Schedule not found'), { statusCode: 404 });
  const allowed = ['name', 'description', 'type', 'timezone', 'businessHours', 'holidays', 'isActive', 'isDefault'];
...
exports.deleteSchedule = async (ctx, scheduleId, actor) => {
  ...
  const schedule = await BusinessSchedule.findOne({ _id: scheduleId, tenantId, isDeleted: false });
  if (!schedule) throw Object.assign(new Error('Schedule not found'), { statusCode: 404 });
  schedule.isDeleted = true; schedule.deletedAt = new Date(); schedule.deletedBy = actor.userId;
  await schedule.save();
  return { success: true };
};
...
exports.createCalendar = async (ctx, data, actor) => {
  ...
  return HolidayCalendar.create({ ...data, tenantId, number, createdBy: actor.userId });
};

exports.updateCalendar = async (ctx, calendarId, data, actor) => {
  const tenantId = requireTenant(ctx);
  const cal = await HolidayCalendar.findOne({ _id: calendarId, tenantId, isDeleted: false });
  if (!cal) throw Object.assign(new Error('Calendar not found'), { statusCode: 404 });
  const allowed = ['name', 'description', 'timezone', 'holidays', 'isActive', 'isDefault'];
...
exports.deleteCalendar = async (ctx, calendarId, actor) => {
  ...
  const cal = await HolidayCalendar.findOne({ _id: calendarId, tenantId, isDeleted: false });
  if (!cal) throw Object.assign(new Error('Calendar not found'), { statusCode: 404 });
  cal.isDeleted = true; cal.deletedAt = new Date(); cal.deletedBy = actor.userId;
  await cal.save();
  return { success: true };
};
...
exports.listConditions = async (ctx, slaPlanId) => {
  const tenantId = requireTenant(ctx);
  return SLACondition.find({ tenantId, slaPlanId, isDeleted: false }).sort({ priority: 1 });
};

exports.createCondition = async (ctx, slaPlanId, data, actor) => {
  const tenantId = requireTenant(ctx);
  return SLACondition.create({ ...data, tenantId, slaPlanId, createdBy: actor.userId });
};

exports.updateCondition = async (ctx, conditionId, data, actor) => {
  const tenantId = requireTenant(ctx);
  const cond = await SLACondition.findOne({ _id: conditionId, tenantId, isDeleted: false });
  if (!cond) throw Object.assign(new Error('Condition not found'), { statusCode: 404 });
  const allowed = ['name', 'description', 'isActive', 'priority', 'conditions', 'conditionGroups'];
...
exports.deleteCondition = async (ctx, conditionId, actor) => {
  ...
  const cond = await SLACondition.findOne({ _id: conditionId, tenantId, isDeleted: false });
  if (!cond) throw Object.assign(new Error('Condition not found'), { statusCode: 404 });
  cond.isDeleted = true; cond.deletedAt = new Date(); cond.deletedBy = actor.userId;
  await cond.save();
  return { success: true };
};
...
exports.getSLAEvents = async (ctx, query = {}) => {
  const tenantId = requireTenant(ctx);
  const filter = { company: tenantId, ...pick(query, ['ticket', 'policy', 'event', 'clock']) };
  return SlaEvent.find(filter).sort({ occurredAt: -1 }).limit(parseInt(query.limit) || 50);
};

exports.getTicketSLATimeline = async (ctx, ticketId) => {
  const tenantId = requireTenant(ctx);
  const events = await SlaEvent.find({ company: tenantId, ticket: ticketId }).sort({ occurredAt: 1 });
  const breakdowns = await SLABreakdown.find({ tenantId, ticketId }).sort({ startedAt: 1 });
...
exports.listRepairJobs = async (ctx, query = {}) => {
  const tenantId = requireTenant(ctx);
  const filter = { tenantId, isDeleted: false, ...pick(query, ['status', 'triggerType', 'slaPlanId']) };
  return SLARepairJob.find(filter).sort({ createdAt: -1 }).limit(parseInt(query.limit) || 50);
};
...
exports.createRepairJob = async (ctx, data, actor) => {
  ...
  const affectedTicketCount = await Ticket.countDocuments({ ...filter, status: { $nin: ['closed', 'archived', 'deleted'] } });
  const job = await SLARepairJob.create({
    ...data, tenantId, number, triggeredBy: actor.userId,
    affectedTicketCount, status: 'pending',
  });
  return job;
};
...
exports.startRepairJob = async (ctx, jobId) => {
  const tenantId = requireTenant(ctx);
  const job = await SLARepairJob.findOne({ _id: jobId, tenantId, isDeleted: false });
...
exports.getDashboard = async (ctx) => {
  ...
  const [plans, totalTickets, openTickets] = await Promise.all([
    ...
    Ticket.countDocuments({ tenantId: tenantId, status: { $nin: ['closed', 'archived', 'deleted'] } }),
  ]);

  const breached = await Ticket.countDocuments({ tenantId: tenantId, isOverdue: true });
  const responseMet = await Ticket.countDocuments({ tenantId: tenantId, responseMetAt: { $exists: true, $ne: null } });
  const responseBreached = await Ticket.countDocuments({ tenantId: tenantId, responseBreached: true });
  const resolutionMet = await Ticket.countDocuments({ tenantId: tenantId, resolutionMetAt: { $exists: true, $ne: null } });

  const responseCompliance = (responseMet + responseBreached) > 0
    ? Math.round((responseMet / (responseMet + responseBreached)) * 100) : 100;
  const resolutionCompliance = resolutionMet > 0 && totalTickets > 0
    ? Math.round((resolutionMet / totalTickets) * 100) : 100;
  ...
  // Recent breaches
  const recentBreaches = await SlaEvent.find({ company: tenantId, event: 'breached' })
    .sort({ occurredAt: -1 }).limit(10).populate('ticket', 'number subject');

  // Active OLAs
  const olas = await OLA.find({ tenantId, isActive: true, isDeleted: false })
    .populate('slaPlanId', 'name').populate('department', 'name');

  return {
    totalPlans: plans.length,
    activePlans: plans.filter(p => p.status !== 'draft').length,
    totalTickets, openTickets, breached,
    responseCompliance, resolutionCompliance,
    planStats, recentBreaches, olas,
  };
};
...
exports.recalculateDueDates = async (ctx, slaPlanId, actor) => {
  const tenantId = requireTenant(ctx);
  const tickets = await Ticket.find({ tenantId: tenantId, sla: slaPlanId, status: { $nin: ['closed', 'archived', 'deleted'] } });
  let updated = 0;
...
exports.measureUCTarget = async (ctx, targetId, actualValue, actor) => {
  ...
  if (met) {
    ...
    emitEvent(tenantId, 'uc.breached', { targetId: target._id, metric: target.metric });
  }
  await target.save();
  return target;
};
