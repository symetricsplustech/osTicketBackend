...
class MajorIncidentService {
  ...
  static async nominate(incidentId, data, tenantId, actorId) {
    ...
    const existingCandidate = await MajorIncidentCandidate.findOne({
      incident: incidentId,
      company: tenantId,
    });
    if (existingCandidate)
      throw new ApiError(
        409,
        "Incident already nominated as major incident candidate",
      );
  ...
  static async approve(candidateId, tenantId, actorId) {
    const candidate = await MajorIncidentCandidate.findOne({
      _id: candidateId,
      company: tenantId,
    });
    if (!candidate) throw new ApiError(404, "Candidate not found");
    if (candidate.status !== "pending")
      throw new ApiError(400, "Candidate is not pending");
    ...
    await CommunicationPlan.create({
      tenantId,
      majorIncidentId: majorIncident._id,
      internal: {
        enabled: true,
        cadenceMinutes: 30,
  ...
  static async demote(majorIncidentId, reason, tenantId, actorId) {
    const majorIncident = await MajorIncident.findOne({
      _id: majorIncidentId,
      company: tenantId,
    });
    if (!majorIncident) throw new ApiError(404, "Major incident not found");
    if (majorIncident.status !== "declared")
      throw new ApiError(400, "Major incident is not declared");
    ...
    await Incident.findByIdAndUpdate(majorIncident.incident, {
      isMajor: false,
    });
  ...
  static async getById(majorIncidentId, tenantId) {
    const mi = await MajorIncident.findOne({
      _id: majorIncidentId,
      company: tenantId,
    })
      .populate("incident")
  ...
  static async listCandidates(tenantId, filters = {}) {
    ...
    const [items, total] = await Promise.all([
      MajorIncidentCandidate.find(query)
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
        .populate("incident", "number title severity status")
  ...
  static async listTeam(majorIncidentId, tenantId) {
    return MajorIncidentTeam.find({ majorIncidentId, isActive: true }).populate(
      "userId",
      "name email role",
    );
  }
  ...
  static async updateCommunicationPlan(
    majorIncidentId,
    planData,
    tenantId,
    actorId,
  ...
  static async deleteTemplate(templateId, tenantId) {
    return CommunicationTemplate.findOneAndDelete({
      _id: templateId,
      tenantId,
    });
  }
  ...
  static async createCommunicationTask(
    majorIncidentId,
    data,
    tenantId,
    actorId,
  ...
  static async executeCommunicationTask(taskId, tenantId) {
    ...
    if (task.status !== "pending" && task.status !== "scheduled")
      throw new ApiError(400, "Task already processed");
  ...
  static async createBridgeSession(majorIncidentId, data, tenantId, actorId) {
    ...
    await MajorIncidentTimelineEvent.create({
      ...
      company: tenantId,
      eventType: "note",
      message: `Bridge session created: ${data.provider}`,
      author: actorId,
      visibility: "internal",
    });
    return session;
  ...
  static async getBridgeSession(majorIncidentId, tenantId) {
    return BridgeSession.findOne({ majorIncidentId, tenantId }).sort({
      createdAt: -1,
    });
  }
  ...
  static async listTimelineEvents(majorIncidentId, tenantId, filters = {}) {
    ...
    return MajorIncidentTimelineEvent.find(query)
      .sort({ createdAt: -1 })
  ...
  static async updateExecSummary(
    majorIncidentId,
    execSummary,
    tenantId,
    actorId,
  ...
  static async processCommunicationCadence(tenantId) {
    ...
    for (const plan of plans) {
      if (plan.internal.enabled && plan.nextInternalAt <= now) {
        ...
        plan.nextInternalAt = new Date(
          now.getTime() + plan.internal.cadenceMinutes * 60000,
        );
      }
      if (plan.external.enabled && plan.nextExternalAt <= now) {
        ...
        plan.nextExternalAt = new Date(
          now.getTime() + plan.external.cadenceMinutes * 60000,
        );
      }
      if (plan.stakeholder.enabled && plan.nextStakeholderAt <= now) {
        ...
        plan.nextStakeholderAt = new Date(
          now.getTime() + plan.stakeholder.cadenceMinutes * 60000,
        );
      }
      if (plan.executive.enabled && plan.nextExecutiveAt <= now) {
        ...
        plan.nextExecutiveAt = new Date(
          now.getTime() + plan.executive.cadenceMinutes * 60000,
        );
      }
      await plan.save();
    }
  ...
  static async broadcastInternal(majorIncidentId, tenantId) {
    ...
    const team = await MajorIncidentTeam.find({
      majorIncidentId,
      isActive: true,
    }).populate("userId", "name email");

    const message =
      mi.communicationPlan?.messageTemplate ||
      `Internal update: ${mi.execSummary || "Status unchanged"}`;
    ...
    const task = await CommunicationTask.create({
      ...
      variables: {
        incidentNumber: mi.incident?.number,
        execSummary: mi.execSummary,
      },
    });

    await this.executeCommunicationTask(task._id, tenantId);
    emitEvent(tenantId, "major_incident.broadcast", {
      majorIncidentId,
      audience: "internal",
      message,
    });
  ...
  static async broadcastStakeholder(majorIncidentId, tenantId) {
    ...
    const message =
      mi.communicationPlan?.messageTemplate ||
      `Stakeholder update: ${mi.execSummary || "Status unchanged"}`;
  ...
  static async getDashboard(tenantId) {
    const [majorIncidents, candidates, plans, templates, bridgeSessions] =
      await Promise.all([
    ...
    for (const mi of majorIncidents) {
      byStatus[mi.status] = (byStatus[mi.status] || 0) + 1;
    }
    ...
    return {
      ...
      byStatus,
      severityBreakdown: majorIncidents.reduce((acc, m) => {
        acc[m.majorType || "other"] = (acc[m.majorType || "other"] || 0) + 1;
        return acc;
      }, {}),
    };
  }
