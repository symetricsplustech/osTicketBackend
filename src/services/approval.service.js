...
async function createApproval({ tenantId, taskId, type, approver, approvalGroup, approvers, requiredApprovals, requestedBy, dueAt }) {
  const task = await Task.findOne({ _id: taskId, tenantId });
  if (!task) throw new ApiError(404, 'Task not found');

  const approval = await Approval.create({
    tenantId, taskId, type, approver, approvalGroup,
    approvers: approvers || [],
  ...
  if (recipient) {
    await NotificationEvent.create({
      tenantId, taskId, eventType: 'approval.requested',
      recipientId: recipient, subject: `Approval required for task ${task.number}`,
  ...
  await TaskActivity.create({
    tenantId, taskId, type: 'system',
    content: `Approval requested (${type})`, actor: requestedBy, isPublic: true,
  });
...
async function decideApproval(approvalId, tenantId, { decision, note, userId }) {
  const approval = await Approval.findOne({ _id: approvalId, tenantId });
  if (!approval) throw new ApiError(404, 'Approval not found');
  if (approval.state !== 'pending') throw new ApiError(422, 'Approval is not pending');

  if (approval.type === 'group' || approval.type === 'parallel') {
    if (!approval.approvers.includes(userId)) throw new ApiError(403, 'You are not an approver');
    if (!approval.approvalsReceived.includes(userId)) {
      approval.approvalsReceived.push(userId);
    }
    const uniqueApprovals = [...new Set(approval.approvalsReceived.map(String))];
    ...
    if (String(approval.approver) !== String(userId)) throw new ApiError(403, 'You are not the approver');
  ...
  await TaskActivity.create({
    tenantId, taskId: approval.taskId, type: 'system',
    content: `Approval ${decision}${note ? ': ' + note : ''}`, actor: userId, isPublic: true,
  });
...
async function delegateApproval(approvalId, tenantId, { delegatedTo, note, userId }) {
  const approval = await Approval.findOne({ _id: approvalId, tenantId });
  if (!approval) throw new ApiError(404, 'Approval not found');
  if (approval.state !== 'pending') throw new ApiError(422, 'Approval is not pending');
  ...
  await NotificationEvent.create({
    tenantId, taskId: approval.taskId, eventType: 'approval.delegated',
    recipientId: delegatedTo, subject: 'Approval delegated to you',
...
module.exports = {
  createApproval, decideApproval, cancelApproval, delegateApproval,
  getApprovalsForTask, getPendingApprovalsForUser, getApprovalStats,
};
