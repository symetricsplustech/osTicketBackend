...
class ProblemService {
  ...
  static async create(data, actorId) {
    ...
    events.emit('problem.created', { problemId: problem._id, tenantId: problem.company });

    return problem;
  }

  static async getById(problemId, tenantId) {
    const problem = await Problem.findOne({ _id: problemId, company: tenantId, isActive: true });
  ...
  static async list(tenantId, filters = {}) {
    ...
    if (filters.assignmentGroup) query.assignmentGroup = filters.assignmentGroup;
    ...
    const [items, total] = await Promise.all([
      Problem.find(query).sort(sort).skip(skip).limit(limit).populate('assignedTo', 'name email').populate('assignmentGroup', 'name'),
  ...
  static async update(problemId, data, tenantId, actorId) {
    ...
    if (data.assignedTo && String(data.assignedTo) !== String(problem.assignedTo)) {
      await ProblemAssignmentHistory.create({
        problem: problem._id,
        company: tenantId,
  ...
  static async transition(problemId, targetStatus, tenantId, actorId, notes = '') {
    const problem = await this.getById(problemId, tenantId);
  ...
  static async assign(problemId, assignment, tenantId, actorId) {
    ...
    if (assignment.assignmentGroup) problem.assignmentGroup = assignment.assignmentGroup;
    problem.updatedBy = actorId;
  ...
  static async linkIncident(problemId, incidentId, tenantId, actorId) {
    const existing = await ProblemIncident.findOne({ problem: problemId, incident: incidentId });
    if (existing) { existing.isActive = true; await existing.save(); return existing; }
  ...
  static async listIncidents(problemId, tenantId) {
    return ProblemIncident.find({ problem: problemId, company: tenantId, isActive: true }).populate('incident');
  }
  ...
  static async linkCI(problemId, ciId, role, tenantId, actorId) {
    ...
    return ProblemCI.create({ problem: problemId, ci: ciId, company: tenantId, role: role || 'affected', linkedBy: actorId });
  }

  static async unlinkCI(problemId, ciId, tenantId) {
    return ProblemCI.findOneAndUpdate({ problem: problemId, ci: ciId, company: tenantId }, { isActive: false }, { new: true });
  }

  static async listCIs(problemId, tenantId) {
    return ProblemCI.find({ problem: problemId, company: tenantId, isActive: true }).populate('ci');
  }

  static async linkServiceOffering(problemId, serviceOfferingId, role, tenantId, actorId) {
    const existing = await ProblemServiceOffering.findOne({ problem: problemId, serviceOffering: serviceOfferingId });
    if (existing) { existing.role = role || existing.role; existing.isActive = true; await existing.save(); return existing; }

    return ProblemServiceOffering.create({ problem: problemId, serviceOffering: serviceOfferingId, company: tenantId, role: role || 'affected', linkedBy: actorId });
  }

  static async linkChange(problemId, changeId, relationshipType, tenantId, actorId) {
    const existing = await ProblemChange.findOne({ problem: problemId, change: changeId });
    if (existing) { existing.relationshipType = relationshipType || existing.relationshipType; existing.isActive = true; await existing.save(); return existing; }

    return ProblemChange.create({ problem: problemId, change: changeId, company: tenantId, relationshipType: relationshipType || 'remediation', linkedBy: actorId });
  }

  static async listChanges(problemId, tenantId) {
    return ProblemChange.find({ problem: problemId, company: tenantId, isActive: true }).populate('change');
  }

  static async linkKnowledge(problemId, knowledgeArticleId, role, tenantId, actorId) {
    const existing = await ProblemKnowledge.findOne({ problem: problemId, knowledgeArticle: knowledgeArticleId });
    if (existing) { existing.role = role || existing.role; existing.isActive = true; await existing.save(); return existing; }

    return ProblemKnowledge.create({ problem: problemId, knowledgeArticle: knowledgeArticleId, company: tenantId, role: role || 'related', linkedBy: actorId });
  }
  ...
  static async createTask(problemId, taskData, tenantId, actorId) {
    ...
    return ProblemTask.create({ ...taskData, number, problem: problemId, company: tenantId, createdBy: actorId });
  }
  ...
  static async publishWorkaround(problemId, tenantId, actorId) {
    ...
    problem.updatedBy = actorId;
    problem.timeline.push({ at: new Date(), by: String(actorId), message: 'Workaround published' });
    await problem.save();
    return problem;
  }

  static async acceptRisk(problemId, reason, tenantId, actorId) {
    return this.transition(problemId, 'risk_accepted', tenantId, actorId, reason);
  }

  static async reanalyze(problemId, tenantId, actorId) {
    return this.transition(problemId, 'assess', tenantId, actorId, 'Re-analyzing');
  }

  static async getAssignmentHistory(problemId, tenantId) {
    return ProblemAssignmentHistory.find({ problem: problemId, company: tenantId }).sort({ assignedAt: -1 });
  }
  ...
  static async listKnownErrors(tenantId, filters = {}) {
    ...
    const [items, total] = await Promise.all([
      KnownError.find(query).sort({ createdAt: -1 }).skip(skip).limit(limit).populate('problem', 'number title'),
  ...
  static async createRootCauseRecord(problemId, data, tenantId, actorId) {
    return RootCauseRecord.create({ ...data, problem: problemId, company: tenantId, identifiedBy: actorId });
  }

  static async listRootCauseRecords(problemId, tenantId) {
    return RootCauseRecord.find({ problem: problemId, company: tenantId }).sort({ identifiedAt: -1 });
  }
}
