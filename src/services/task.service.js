...
async function createNotification({
  ...
  return NotificationEvent.create({
    ...
    logger.error("Notification create failed", { error: err.message }),
  );
}
...
async function getWatchers(taskId, tenantId) {
  return TaskWatcher.find({ tenantId, taskId }).populate(
    "userId",
    "name email avatar",
  );
}
...
async function createRelationship({
  ...
  if (String(sourceTaskId) === String(targetTaskId)) {
    throw new ApiError(
      400,
      "Cannot create relationship between a task and itself",
    );
  }
...
async function getRelationships(taskId, tenantId) {
  const outgoing = await TaskRelationship.find({
    tenantId,
    sourceTaskId: taskId,
  }).populate("targetTaskId", "number title state priority assignedTo");
  const incoming = await TaskRelationship.find({
    tenantId,
    targetTaskId: taskId,
...
async function createTask({
  ...
  if (assignmentGroup) {
    await createNotification({
      tenantId,
      taskId: task._id,
      eventType: "task.group_assigned",
      recipientId: assignmentGroup,
      subject: `Task ${number} assigned to group`,
...
async function updateTask(taskId, tenantId, updates, actorId) {
  ...
  for (const key of allowedFields) {
    if (
      ...
      JSON.stringify(updates[key]) !== JSON.stringify(task[key])
    ) {
  ...
  if (changes.assignedTo && changes.assignedTo.to) {
    ...
    await createNotification({
      ...
      taskId: task._id,
      eventType: "task.assigned",
      recipientId: changes.assignedTo.to,
      subject: `Task ${task.number} assigned to you`,
  ...
  for (const w of watchers) {
    if (String(w.userId) !== String(actorId)) {
      await createNotification({
        ...
        subject: `Task ${task.number} state changed`,
...
async function addComment(
  taskId,
  tenantId,
  { content, actorId, isPublic = true },
) {
  const task = await Task.findOne({ _id: taskId, tenantId });
  if (!task) throw new ApiError(404, "Task not found");
  if (TASK_TERMINAL.includes(task.state))
    throw new ApiError(422, "Cannot add comment to terminal task");
