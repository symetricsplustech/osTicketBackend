...
async function uploadAttachment({
  ...
  if (file.size > MAX_SIZE)
    throw new ApiError(400, `File exceeds maximum size of ${MAX_SIZE} bytes`);
  ...
  await TaskActivity.create({
    ...
    taskId,
    type: "attachment",
    content: `Attached: ${file.originalname}`,
    actor: uploadedBy,
    isPublic,
  });
...
async function listAttachments(
  ...
  return TaskAttachment.find(query)
    .sort({ createdAt: -1 })
...
async function softDeleteAttachment(attachmentId, tenantId, deletedBy) {
  ...
  await TaskActivity.create({
    ...
    taskId: att.taskId,
    type: "system",
    content: `Deleted attachment: ${att.originalName}`,
    actor: deletedBy,
    isPublic: false,
  });
