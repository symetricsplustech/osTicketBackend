...
function formatWebhookBody(target, eventName, payload = {}) {
  if (target.format !== 'slack' && target.format !== 'teams') {
    return { body: JSON.stringify({ event: eventName, data: payload, sentAt: new Date().toISOString() }), contentType: 'application/json' };
  }
  const d = payload || {};
  const label = String(eventName).replace(/^ticket\./, 'Ticket ').replace(/_/g, ' ');
  const bits = [label];
...
async function dispatchWebhooks(eventName, payload) {
  try {
    ...
    for (const target of targets) {
      const { body, contentType } = formatWebhookBody(target, eventName, payload);
