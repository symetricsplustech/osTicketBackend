...
function matchesCondition(cond, ticket) {
  ...
  switch (cond.operator) {
    ...
    case "lte":
      return val <= cond.value;
    case "starts_with":
      return typeof val === "string" && val.startsWith(cond.value);
    case "ends_with":
      return typeof val === "string" && val.endsWith(cond.value);
    case "is_empty":
      return !val;
    case "is_not_empty":
      return !!val;
...
function matchesConditions(conditions, ticket) {
  ...
  if (conditions.matchAll)
    return conditions.rules.every((r) => matchesCondition(r, ticket));
...
exports.getRule = async (ctx, ruleId) => {
  ...
  if (!rule)
    throw Object.assign(new Error("Assignment rule not found"), {
      statusCode: 404,
    });
  return rule;
};
...
exports.listQueues = async (ctx, query = {}) => {
  ...
  const filter = {
    tenantId,
    isDeleted: false,
    ...pick(query, ["isActive", "type", "department"]),
  };
...
exports.getQueueStats = async (ctx, queueId) => {
  ...
  return {
    ...
    slaBreachedCount: queue.slaBreachedCount,
...
exports.listSkills = async (ctx, query = {}) => {
  ...
  const filter = {
    tenantId,
    isDeleted: false,
    ...pick(query, ["isActive", "category", "group"]),
  };
  if (query.search)
    filter.$or = [{ name: { $regex: query.search, $options: "i" } }];
...
exports.listAgentSkills = async (ctx, agentId) => {
  const tenantId = requireTenant(ctx);
  return AgentSkill.find({ tenantId, agentId, isActive: true }).populate(
    "skillId",
    "name category",
  );
};
...
exports.getPresence = async (ctx, agentId) => {
  const tenantId = requireTenant(ctx);
  const latest = await AgentPresence.findOne({ tenantId, agentId }).sort({
    createdAt: -1,
  });
...
exports.getPresenceHistory = async (ctx, agentId, query = {}) => {
  const tenantId = requireTenant(ctx);
  return AgentPresence.find({ tenantId, agentId })
    .sort({ createdAt: -1 })
...
exports.setPresence = async (ctx, agentId, data) => {
  const tenantId = requireTenant(ctx);
  const previous = await AgentPresence.findOne({ tenantId, agentId }).sort({
    createdAt: -1,
  });
  ...
  emitEvent(tenantId, "agent.presenceChanged", {
    agentId,
    status: data.status,
    previousStatus: previous?.status,
  });
...
exports.getPresenceStats = async (ctx) => {
  ...
  for (const a of agents) {
    statusCounts[a.presence || "offline"] =
      (statusCounts[a.presence || "offline"] || 0) + 1;
  }
...
exports.getCapacity = async (ctx, agentId) => {
  ...
  if (!cap)
    cap = await AgentCapacity.create({
      tenantId,
      agentId,
      maxCapacity: 10,
      currentLoad: 0,
...
exports.updateCapacity = async (ctx, agentId, data) => {
  ...
  let cap = await AgentCapacity.findOne({ tenantId, agentId });
  if (!cap)
    cap = await AgentCapacity.create({ tenantId, agentId, maxCapacity: 10 });
...
exports.listRoutingRules = async (ctx, query = {}) => {
  ...
  const filter = {
    tenantId,
    isDeleted: false,
    ...pick(query, ["isActive", "source", "department"]),
  };
...
exports.getRoutingRule = async (ctx, ruleId) => {
  ...
  if (!rule)
    throw Object.assign(new Error("Routing rule not found"), {
      statusCode: 404,
    });
  return rule;
};
...
exports.getHistoryByNumber = async (ctx, ticketNumber) => {
  const tenantId = requireTenant(ctx);
  return AssignmentEvent.find({ tenantId, ticketNumber }).sort({
    createdAt: -1,
  });
};
...
exports.getRecentEvents = async (ctx, query = {}) => {
  ...
  const filter = {
    tenantId,
    ...pick(query, ["eventType", "fromAgent", "toAgent", "method"]),
  };
  return AssignmentEvent.find(filter)
    .sort({ createdAt: -1 })
    .limit(parseInt(query.limit) || 50);
};
...
exports.findBestAgent = async (ctx, options = {}) => {
  ...
  const available = candidates.filter((a) =>
    ["available", "busy"].includes(a.presence),
  );
  ...
  switch (strategy) {
    ...
    case "least_workload":
      return available.sort(
        (a, b) =>
          (a.lockedTickets?.length || 0) - (b.lockedTickets?.length || 0),
      )[0];
    case "skill_based":
      return available[0];
    case "availability":
...
exports.offerWork = async (ctx, ticketId, agentId, actor) => {
  ...
  emitEvent(tenantId, "work.offered", {
    ticketId,
    agentId,
    eventId: event._id,
  });
...
exports.diagnose = async (ctx, ticket) => {
  ...
  const aRules = await AssignmentRule.find({
    tenantId,
    isActive: true,
    isDeleted: false,
  }).sort({ priority: -1 });
  for (const r of aRules) {
    if (matchesConditions(r.conditions, ticket))
      results.rules.push({ id: r._id, name: r.name, matched: true });
    else results.rules.push({ id: r._id, name: r.name, matched: false });
  }

  // Check routing rules
  const rRules = await RoutingRule.find({
    tenantId,
    isActive: true,
    isDeleted: false,
  }).sort({ priority: -1 });
  for (const r of rRules) {
    if (matchesConditions(r.conditions, ticket))
      results.routingRules.push({ id: r._id, name: r.name, matched: true });
    else results.routingRules.push({ id: r._id, name: r.name, matched: false });
  }

  // Find best agent
  results.bestAgent = await exports.findBestAgent(ctx, {
