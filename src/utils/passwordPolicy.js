...
async function assertPasswordPolicy(password, companyId) {
  ...
  const errors = [];
  if (pw.length < (pol.minLength || 8))
    errors.push(`Minimum ${pol.minLength || 8} characters`);
  if (pol.requireUpper && !/[A-Z]/.test(pw))
    errors.push("Needs an uppercase letter");
  if (pol.requireNumber && !/[0-9]/.test(pw)) errors.push("Needs a number");
  if (pol.requireSymbol && !/[^A-Za-z0-9]/.test(pw))
    errors.push("Needs a symbol");
  if (errors.length)
    throw new ApiError(422, `Password policy: ${errors.join(", ")}`);
  return pol;
}
