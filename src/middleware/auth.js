...
const attachActiveCompany = async (principal, req) => {
    ...
    try {
      require("../services/audit.service")
        .audit({
          ...
          actorType: principal.isAdmin !== undefined ? "agent" : "user",
          actor: principal._id,
          actorName: principal.name,
          action: "tenant.access_denied",
          entityType: "tenant",
          entityId: principal.company || principal._id,
          after: { reason },
          source: "auth",
          req,
        })
  ...
  if (!company.isActive()) {
    ...
    throw new ApiError(
      403,
      `This tenant is not active (status: ${company.status})`,
    );
  }
...
const protectAdmin = asyncHandler(async (req, res, next) => {
  ...
  if (
    !agent.isAdmin &&
    !(agent.role && agent.role.isAdmin) &&
    agent.role?.category !== "auditor"
  ) {
...
const requirePermission = (perm, opts = {}) =>
  asyncHandler(async (req, res, next) => {
    ...
    const record =
      typeof opts.record === "function" ? await opts.record(req) : opts.record;
...
const requirePlatformPermission = (...perms) =>
  asyncHandler(async (req, res, next) => {
    const superAdmin = req.superAdmin;
    if (!superAdmin) throw new ApiError(401, "Not authorized");
    if (!superAdmin.platformRole)
      throw new ApiError(
        403,
        "Platform role migration required for this account",
      );
