...
const protectApiKey = asyncHandler(async (req, res, next) => {
  ...
  if (!header.startsWith("Bearer "))
    throw new ApiError(401, "API key required (Bearer ost_...)");
  ...
  req.apiKey = key;
  if (!key.company)
    throw new ApiError(403, "API key is not assigned to a tenant");
  req.companyId = key.company;
  req.apiScopes = key.scopes || [];
  next();
});

const hasScope = (req, scope) =>
  (req.apiScopes || []).includes(scope) || (req.apiScopes || []).includes("*");
