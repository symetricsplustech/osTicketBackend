...
router.get(
  ...
  ctrl.listFaqCategories,
);
router.post(
  "/faq-categories",
  moduleRequired("helpdesk"),
  requirePermission("kb.manage"),
  ctrl.createFaqCategory,
);
router.get(
  "/faqs",
  moduleRequired("helpdesk"),
  ...
  async (req, res, next) => {
    try {
      ...
      const articles = faqs.map((faq) => ({
        ...
        deflectionRate: faq.views
          ? Math.round(((faq.helpful || 0) / faq.views) * 100)
          : 0,
      }));
      ...
      const totalDeflected = articles.reduce(
        (sum, article) => sum + article.votesUp,
        0,
      );
      res.json({
        totalViews,
        totalDeflected,
        overallDeflectionRate: totalViews
          ? Math.round((totalDeflected / totalViews) * 100)
          : 0,
        articles,
      });
    } catch (error) {
