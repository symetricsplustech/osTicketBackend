...
router.get(
  ...
  async (req, res, next) => {
    try {
      const {
        predictBreachMany,
      } = require("../../../services/slaPredictor.service");
      const { scopeTicketQuery } = require("../../../controllers/helpdesk");
      const result = await predictBreachMany({
        company: req.companyId,
        limit: parseInt(req.query.limit, 10) || 50,
        scope: scopeTicketQuery(req.agent),
      });
      res.json(result);
    } catch (e) {
      next(e);
...
router.get(
  ...
  async (req, res, next) => {
    try {
      const {
        semanticSearch,
      } = require("../../../services/neuralSearch.service");
...
router.get(
  "/tickets/:number/sla-history",
  require("../../../controllers/admin.controller").ticketSlaHistory,
);
...
router.delete(
  "/tickets/:number/collaborators/:userId",
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const result = await predictBreach({
        company: req.companyId,
        ticketId: ticket._id,
      });
      res.json(result);
...
router.post(
  "/tickets/:number/tasks",
  moduleRequired("helpdesk"),
  requirePermission("tickets.tasks"),
  ctrl.addTask,
);
router.put(
  "/tickets/:number/tasks/:taskId",
  moduleRequired("helpdesk"),
  requirePermission("tickets.tasks"),
  ctrl.updateTask,
);
...
router.post(
  "/tickets/suggest",
  moduleRequired("helpdesk"),
  assistCtrl.suggestTriage,
);
router.get(
  "/tickets/suggest-refs",
  moduleRequired("helpdesk"),
  assistCtrl.suggestRefs,
);
router.post(
  "/assist/summarize",
  moduleRequired("helpdesk"),
  assistCtrl.summarize,
);
