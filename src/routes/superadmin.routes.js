...
router.get(
  "/dashboard",
  requirePlatformPermission(P.DASHBOARD_READ),
  ctrl.dashboard,
);
router.get(
  "/stats",
...
router.post(
  "/plans",
  requirePlatformPermission(P.PLAN_CREATE),
  ctrl.createPlan,
);
router.put(
  "/plans/:id",
  requirePlatformPermission(P.PLAN_UPDATE),
  ctrl.updatePlan,
);
router.delete(
  "/plans/:id",
  requirePlatformPermission(P.PLAN_DISABLE),
  ctrl.deletePlan,
);
...
router.get(
  "/companies",
  requirePlatformPermission(P.TENANT_READ),
  ctrl.listCompanies,
);
router.post(
  "/companies",
  requirePlatformPermission(P.TENANT_CREATE),
  ctrl.createCompany,
);
router.get(
  "/companies/:id",
...
router.put(
  "/companies/:id",
  requirePlatformPermission(P.TENANT_UPDATE),
  ctrl.updateCompany,
);
router.delete(
  "/companies/:id",
  requirePlatformPermission(P.TENANT_TERMINATE),
  ctrl.deleteCompany,
);
router.put(
  "/companies/:id/status",
  ...
  ctrl.changeCompanyStatus,
);
router.put(
  "/companies/:id/plan",
  requirePlatformPermission(P.PLAN_ASSIGN),
  ctrl.changeCompanyPlan,
);
router.get(
  "/companies/:id/admins",
  requirePlatformPermission(P.TENANT_MANAGE_ADMIN),
  ctrl.listCompanyAdmins,
);
router.post(
  "/companies/:id/admins",
  requirePlatformPermission(P.TENANT_MANAGE_ADMIN),
  ctrl.createCompanyAdmin,
);
router.put(
  "/companies/:id/admins/:adminId",
  requirePlatformPermission(P.TENANT_MANAGE_ADMIN),
  ctrl.updateCompanyAdmin,
);
router.post(
  "/companies/:id/admins/:adminId/reset-password",
  requirePlatformPermission(P.TENANT_MANAGE_ADMIN),
  ctrl.resetCompanyAdminPassword,
);
router.put(
  "/companies/:id/modules",
  requirePlatformPermission(P.TENANT_MANAGE_MODULES),
  ctrl.updateCompanyModules,
);

// Invoices
router.get(
  "/invoices",
  requirePlatformPermission(P.BILLING_READ),
  ctrl.listInvoices,
);
router.get(
  "/subscriptions",
...
router.put(
  "/subscriptions/:id",
  requirePlatformPermission(P.BILLING_MANAGE),
  ctrl.updateSubscription,
);
router.get(
  "/billing-adjustments",
  requirePlatformPermission(P.BILLING_READ),
  ctrl.listBillingAdjustments,
);
router.post(
  "/billing-adjustments",
  requirePlatformPermission(P.BILLING_MANAGE),
  ctrl.createBillingAdjustment,
);
...
router.post(
  "/impersonate",
  requirePlatformPermission(P.SUPPORT_IMPERSONATE),
  ctrl.impersonateCompanyAdmin,
);
router.post(
  "/break-glass",
  requirePlatformPermission(P.SECURITY_BREAK_GLASS),
  ctrl.breakGlassAccess,
);
router.get(
  "/privileged-sessions",
  requirePlatformPermission(P.SECURITY_READ),
  ctrl.listPrivilegedSessions,
);
router.post(
  "/privileged-sessions/:sessionId/revoke",
  requirePlatformPermission(P.SESSION_REVOKE),
  ctrl.revokePrivilegedSession,
);

// Super admin management (platform owner only)
router.get(
  "/admins",
  requirePlatformPermission(P.ADMIN_MANAGE),
  ctrl.listSuperAdmins,
);
router.post(
  "/admins",
  requirePlatformPermission(P.ADMIN_MANAGE),
...
router.put(
  "/admins/:id",
  requirePlatformPermission(P.ADMIN_MANAGE),
  ctrl.updateSuperAdmin,
);
router.delete(
  "/admins/:id",
  requirePlatformPermission(P.ADMIN_MANAGE),
  ctrl.deleteSuperAdmin,
);

// Platform settings
router.get(
  "/settings",
  requirePlatformPermission(P.PLATFORM_CONFIGURE),
  ctrl.getSettings,
);
router.put(
  "/settings",
...
router.get(
  "/operations/health",
  requirePlatformPermission(P.OPERATIONS_READ),
      ...
      data: {
        ...
        services: {
          database: {
            ...
            state: healthStates[dbState] || "unknown",
          },
...
router.get(
  "/modules",
  requirePlatformPermission(P.MODULE_READ),
  asyncHandler(async (req, res) => {
    ...
    const result = allModuleKeys.map((key) => {
      ...
      return {
        ...
        totalTenants: found?.tenants || 0,
        activeTenants: found?.active || 0,
      };
    });
...
router.put(
  ...
  requirePlatformPermission(P.MODULE_CONFIGURE),
  asyncHandler(async (req, res) => {
    ...
    const result = await db
      .collection("tenant_modules")
      .updateMany({ moduleKey }, { $set: { status, updatedAt: new Date() } });
    res.json({
      success: true,
      data: { moduleKey, status, affectedTenants: result.modifiedCount },
    });
  }),
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const admins = await Agent.find({
      isActive: true,
      $or: [{ isAdmin: true }, { "role.isAdmin": true }],
    })
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!sessionId || !reason)
      throw new ApiError(422, "sessionId and reason are required");
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!userId || !reason)
      throw new ApiError(422, "userId and reason are required");
    ...
    const candidates = principalType
    ...
    const PrivilegedSession = require("../models/PrivilegedSession");
    await PrivilegedSession.updateMany(
      { effectiveActor: userId, status: "active" },
      {
        $set: {
          status: "revoked",
          revokedAt: new Date(),
    ...
    res.json({
      success: true,
      message: `All ${matchedType} sessions revoked`,
      reason,
    });
  }),
);
...
router.get(
  "/audit-logs/export",
  requirePlatformPermission(P.AUDIT_EXPORT),
  asyncHandler(async (req, res) => {
    const AuditLog = require("../models/AuditLog");
    const q = {};
    ...
    const items = await AuditLog.find(q)
      ...
      .sort({ createdAt: -1 })
      .limit(5000);
    const csv = ["Date,Action,Entity,Entity ID,Actor,Company,IP"]
      .concat(
        items.map(
          (i) =>
            `${new Date(i.createdAt).toISOString()},${i.action},${i.entityType || ""},${i.entityId || ""},${i.superAdmin?.name || ""},${i.company?.name || ""},${i.ip || ""}`,
        ),
      )
      .join("\n");
