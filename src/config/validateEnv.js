...
function validateEnv() {
  ...
  for (const key of RECOMMENDED) {
    if (!process.env[key])
      warnings.push(`Missing recommended env var: ${key} (using default)`);
  }
  if (process.env.NODE_ENV === "production") {
    for (const key of SECURITY) {
      if (!process.env[key])
        errors.push(`Missing SECURITY env var in production: ${key}`);
    }
    ...
    if (!process.env.CORS_ORIGINS) {
      warnings.push(
        "CORS_ORIGINS not set — API will reject all browser origins except same-domain",
      );
    }
  }
  if (errors.length) {
