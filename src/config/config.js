...
const config = {
  ...
  mongoUri:
    process.env.MONGODB_URI || "mongodb://127.0.0.1:27017/osticket_mern",
  jwt: {
  ...
  workflow: {
    timerIntervalMinutes:
      parseInt(process.env.WORKFLOW_TIMER_INTERVAL_MINUTES, 10) || 5,
  },
  sla: {
    overdueIntervalMinutes:
      parseInt(process.env.SLA_OVERDUE_INTERVAL_MINUTES, 10) || 5,
    warningThresholdHours: parseFloat(
      process.env.SLA_WARNING_THRESHOLD_HOURS || "2",
    ),
  },
};
