...
const newSession = async ({
  ...
  const ttl = Math.min(
    Math.max(Number(ttlMin) || SESSION_TTL_MIN, 1),
    SESSION_TTL_MAX_MIN,
  );
  ...
  const session = await PrivilegedSession.create({
    ...
    approvedBy: breakGlass
      ? `self:${req.superAdmin.email || req.superAdmin._id}`
      : "",
...
exports.impersonateCompanyAdmin = asyncHandler(async (req, res) => {
  ...
  if (!reason || !String(reason).trim())
    throw new ApiError(422, "A support reason is required for impersonation");
  ...
  const { session, ttl } = await newSession({
    kind: "impersonation",
    req,
    company,
    admin,
  ...
  await log(req, "impersonation.company_admin", "Company", companyId, {
    ...
    realActor: session.realActorEmail,
...
exports.breakGlassAccess = asyncHandler(async (req, res) => {
  const { companyId, reason, ttlMinutes } = req.body;
  if (!reason || !String(reason).trim())
    throw new ApiError(422, "A reason is required for break-glass access");
  const company = await Company.findById(companyId);
  if (!company) throw new ApiError(404, "Company not found");
  const admin = await Agent.findOne({
    company: companyId,
  ...
  await notifyAllSAs({
    type: "break_glass",
    message: `Break-glass access by ${session.realActorEmail}: ${session.reason}`,
    companyId,
  });
  res.json({
    success: true,
...
exports.listPrivilegedSessions = asyncHandler(async (req, res) => {
  ...
  const sessions = await PrivilegedSession.find(q)
    .sort({ createdAt: -1 })
    .limit(200);
...
exports.revokePrivilegedSession = asyncHandler(async (req, res) => {
  const session = await PrivilegedSession.findOne({
  ...
  session.terminationReason =
    terminationReason || "revoked by platform operator";
  await session.save();
  await log(
    req,
