...
exports.dashboard = asyncHandler(async (req, res) => {
  ...
  const [
    ...
    Ticket.countDocuments({
      status: { $nin: ["closed", "resolved", "deleted", "archived"] },
    }),
    ...
    Company.find({
      ...
      .populate("plan", "priceMonthly priceYearly")
  ...
  const plansList = await Plan.find({ isActive: true }).sort({
    priceMonthly: 1,
  });
  ...
  res.json({
    ...
    data: {
      counts: {
        ...
        slaBreaches,
        failedInvoices,
        mrr: subscribedCompanies.reduce(
          (sum, company) =>
            sum +
            (company.billingCycle === "yearly"
              ? Number(company.plan?.priceYearly || 0) / 12
              : Number(company.plan?.priceMonthly || 0)),
          0,
        ),
        arr: subscribedCompanies.reduce(
          (sum, company) =>
            sum +
            (company.billingCycle === "yearly"
              ? Number(company.plan?.priceYearly || 0)
              : Number(company.plan?.priceMonthly || 0) * 12),
          0,
        ),
      },
      tenantStatuses: Object.fromEntries(
        tenantStatuses.map((row) => [row._id, row.count]),
      ),
      companyDistribution,
      recentCompanies,
      recentInvoices,
...
exports.auditLogs = asyncHandler(async (req, res) => {
  ...
  const q = {};
  if (req.query.action)
    q.action = { $regex: String(req.query.action), $options: "i" };
