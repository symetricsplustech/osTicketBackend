...
exports.getCompany = asyncHandler(async (req, res) => {
  ...
  res.json({
    success: true,
    data: { ...company.toObject(), ...meta, invoices },
  });
});
exports.getCompanyStructure = asyncHandler(async (req, res) => {
  ...
  const tenantModules = await mongoose2.connection.db
  ...
  const [agentTicketCounts, userTicketCounts, openAgentTickets] =
    await Promise.all([
      Ticket.aggregate([
        ...
        { $match: { user: { $in: userIds }, company: company._id } },
        {
          $group: {
            ...
            open: {
              $sum: {
                $cond: [
                  { $in: ["$status", ["open", "assigned", "overdue"]] },
                  1,
                  0,
                ],
              },
  ...
  const agentTickets = Object.fromEntries(
    agentTicketCounts.map((x) => [String(x._id), x.total]),
  );
  const agentOpen = Object.fromEntries(
    openAgentTickets.map((x) => [String(x._id), x.total]),
  );
  const userTickets = Object.fromEntries(
    userTicketCounts.map((x) => [
      String(x._id),
  ...
  res.json({
    ...
    data: {
      ...
      departments: departments.map((d) => ({
        ...
        isPublic: d.isPublic,
        manager: d.manager
          ? { _id: d.manager._id, name: d.manager.name }
          : null,
        autoAssignAgent: d.autoAssignAgent
          ? { _id: d.autoAssignAgent._id, name: d.autoAssignAgent.name }
          : null,
      })),
      teams: teams.map((t) => ({
        _id: t._id,
      ...
      users: users.map((u) => ({
        ...
        organization: u.organization
          ? { _id: u.organization._id, name: u.organization.name }
          : null,
      ...
      organizations: orgs.map((o) => ({
        ...
        customers: users.filter(
          (u) => String(u.organization?._id) === String(o._id),
        ).length,
      })),
    },
  });
...
exports.createCompany = asyncHandler(async (req, res) => {
  ...
  const activePlan = plan
    ...
    : await Plan.findOne({ isDefault: true, isActive: true });
    ...
    await Subscription.findOneAndUpdate(
      { company: company._id },
      {
        $set: {
          ...
          currentPeriodEnd: company.planExpiresAt,
  ...
  const finalAdminEmail =
    adminEmail ||
    `admin@${(name || "company").toLowerCase().replace(/[^a-z0-9]/g, "")}.com`;
  ...
  const moduleKeys =
    Array.isArray(modules) && modules.length > 0
      ? modules
      : activePlan?.moduleKeys || [];
  const finalModuleKeys = moduleKeys.length > 0 ? moduleKeys : DEFAULT_MODULES;
  if (!finalModuleKeys.includes("helpdesk"))
    finalModuleKeys.unshift("helpdesk");
  ...
  await log(req, "company.created", "Company", company._id, {
    name,
    modules: moduleKeys,
  });
  await notifySA({
  ...
  res.status(201).json({
    ...
    data: {
      ...company.toObject(),
      admin: createdAdmin
        ? { email: createdAdmin.email, password: finalAdminPassword }
        : null,
    },
  });
});
exports.updateCompany = asyncHandler(async (req, res) => {
  ...
  await company.save();
  await log(req, "company.updated", "Company", company._id, {
    name: company.name,
  });
  await notifySA({
    superAdminId: req.superAdmin._id,
...
exports.deleteCompany = asyncHandler(async (req, res) => {
  ...
  if (company.legalHold)
    throw new ApiError(
      409,
      "Tenant is under legal hold and cannot be terminated",
    );
  const reason = String(req.body?.reason || "").trim();
  if (!reason) throw new ApiError(422, "A termination reason is required");
  const retentionDays = Math.max(
    1,
    Math.min(Number(req.body?.retentionDays) || 30, 3650),
  );
  ...
  company.lifecycleHistory.push({
    status: "terminated",
    reason,
    actor: req.superAdmin._id,
  });
  await company.save();
  await log(req, "company.terminated", "Company", company._id, {
    name: company.name,
    reason,
    purgeScheduledAt: company.purgeScheduledAt,
  });
  await notifyAllSAs({
  ...
  res.json({
    success: true,
    data: company,
    message: "Tenant terminated and retained for recovery",
  });
});
exports.changeCompanyStatus = asyncHandler(async (req, res) => {
  ...
  if (status !== "active" && !reason)
    throw new ApiError(422, "A reason is required for this status change");
  ...
  await company.save();
  await log(req, `company.${status}`, "Company", company._id, {
    name: company.name,
  });
  await notifySA({
    superAdminId: req.superAdmin._id,
...
exports.createCompanyAdmin = asyncHandler(async (req, res) => {
  ...
  if (await Agent.exists({ email: normalizedEmail }))
    throw new ApiError(409, "An agent with this email already exists");
  const generatedPassword =
    password || require("crypto").randomBytes(15).toString("base64url");
  ...
  await log(req, "company.admin_created", "Agent", admin._id, {
    companyId: company._id,
    email: normalizedEmail,
    makeOwner: !!makeOwner,
  });
...
exports.updateCompanyAdmin = asyncHandler(async (req, res) => {
  ...
  await log(req, "company.admin_updated", "Agent", admin._id, {
...
exports.resetCompanyAdminPassword = asyncHandler(async (req, res) => {
  ...
  const temporaryPassword =
    req.body.password ||
    require("crypto").randomBytes(15).toString("base64url");
  admin.password = temporaryPassword;
  admin.sessionVersion = Number(admin.sessionVersion || 0) + 1;
  await admin.save();
  await log(req, "company.admin_password_reset", "Agent", admin._id, {
    email: admin.email,
  });
  res.json({
    success: true,
    temporaryPassword: req.body.password ? undefined : temporaryPassword,
    message: "Password reset and existing sessions revoked",
  });
});
...
exports.updateCompanyModules = asyncHandler(async (req, res) => {
  ...
  await log(req, "company.modules_updated", "Company", company._id, {
    name: company.name,
    modules: activated,
  });
  ...
  res.json({
    success: true,
    data: { companyId: company._id, modules: activated },
  });
});
