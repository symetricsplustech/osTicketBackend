...
const PLATFORM_ROLES = [
  ...
  "platform_billing_administrator",
  "platform_security_administrator",
  "platform_developer_administrator",
  "platform_auditor",
];

const assertAnotherOwnerRemains = async (
  target,
  nextRole,
  ...
  if (
    ...
    (nextRole === "platform_owner" && nextActive)
  )
    return;
  const others = await SuperAdmin.countDocuments({
    _id: { $ne: target._id },
    platformRole: "platform_owner",
    isActive: true,
  });
  if (!others)
    throw new ApiError(409, "At least one active Platform Owner must remain");
};

exports.createSuperAdmin = asyncHandler(async (req, res) => {
  const { name, email, password, role, platformRole, permissions, moduleKeys } =
    req.body;
  if (!name || !email || !password)
    throw new ApiError(400, "Name, email and password are required");
  if (platformRole !== undefined && !PLATFORM_ROLES.includes(platformRole))
    throw new ApiError(422, "Invalid platform role");
  const exists = await SuperAdmin.findOne({ email: email.toLowerCase() });
  if (exists)
    throw new ApiError(409, "A super admin with this email already exists");
...
exports.deleteSuperAdmin = asyncHandler(async (req, res) => {
  ...
  await sa.deleteOne();
  await log(req, "superadmin.deleted", "SuperAdmin", req.params.id, {
    name: sa.name,
  });
