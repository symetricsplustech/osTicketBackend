...
exports.decide = async (req, res, next) => {
  try {
    ...
    const approval = await approvalService.decideApproval(req.params.id, tenantId, {
...
exports.cancel = async (req, res, next) => {
  try {
    const tenantId = req.user?.company || req.companyId;
    const approval = await approvalService.cancelApproval(req.params.id, tenantId, req.user._id);
    ok(res, approval);
  } catch (err) { next(err); }
};
...
exports.delegate = async (req, res, next) => {
  try {
    ...
    const approval = await approvalService.delegateApproval(req.params.id, tenantId, {
      delegatedTo, note, userId: req.user._id,
    });
    ok(res, approval);
  } catch (err) { next(err); }
};
...
exports.listForTask = async (req, res, next) => {
  try {
    const tenantId = req.user?.company || req.companyId;
    const approvals = await approvalService.getApprovalsForTask(req.params.taskId, tenantId);
    ok(res, approvals);
  } catch (err) { next(err); }
};
...
exports.pendingForUser = async (req, res, next) => {
  try {
    const tenantId = req.user?.company || req.companyId;
    const approvals = await approvalService.getPendingApprovalsForUser(req.user._id, tenantId);
    ok(res, approvals);
  } catch (err) { next(err); }
};
