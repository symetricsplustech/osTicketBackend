...
exports.createFaqCategory = asyncHandler(async (req, res) => {
  ...
  if (!String(name || '').trim()) throw new ApiError(422, 'Category name is required');
  ...
  await auditRequired({ company: req.companyId, actorType: 'agent', actor: req.agent._id, actorName: req.agent.name, action: 'knowledge.category_created', entityType: 'faq_category', entityId: cat._id, after: { name: cat.name, isPublic: cat.isPublic }, req });
...
exports.createFaq = asyncHandler(async (req, res) => {
  if (!hasPerm(req.agent, 'kb.manage')) throw new ApiError(403, 'Permission denied');
  const { category, question, answer, keywords, internalOnly, visibility, locale } = req.body;
  if (!question || !answer) throw new ApiError(422, 'Question and answer are required');
  if (category && !await FaqCategory.exists({ _id: category, company: req.companyId })) throw new ApiError(404, 'FAQ category not found');
  if (visibility && !['public', 'customers', 'employees', 'agents', 'department', 'team'].includes(visibility)) throw new ApiError(422, 'Invalid FAQ visibility');
  ...
  await auditRequired({ company: req.companyId, actorType: 'agent', actor: req.agent._id, actorName: req.agent.name, action: 'knowledge.article_created', entityType: 'faq', entityId: faq._id, after: { question: faq.question, lifecycle: faq.lifecycle, visibility: faq.visibility }, req });
...
exports.updateFaq = asyncHandler(async (req, res) => {
  ...
  const before = { question: faq.question, answer: faq.answer, category: faq.category, keywords: faq.keywords, lifecycle: faq.lifecycle, visibility: faq.visibility, internalOnly: faq.internalOnly, locale: faq.locale };
  const { category, question, answer, keywords, internalOnly, visibility, locale } = req.body;
  if (category && !await FaqCategory.exists({ _id: category, company: req.companyId })) throw new ApiError(404, 'FAQ category not found');
  if (visibility && !['public', 'customers', 'employees', 'agents', 'department', 'team'].includes(visibility)) throw new ApiError(422, 'Invalid FAQ visibility');
  ...
  await faq.save();
  await auditRequired({ company: req.companyId, actorType: 'agent', actor: req.agent._id, actorName: req.agent.name, action: 'knowledge.article_updated', entityType: 'faq', entityId: faq._id, before, after: { question: faq.question, answer: faq.answer, category: faq.category, keywords: faq.keywords, lifecycle: faq.lifecycle, visibility: faq.visibility, internalOnly: faq.internalOnly, locale: faq.locale }, req });
  res.json({ success: true, faq });
});
exports.deleteFaq = asyncHandler(async (req, res) => {
  if (!hasPerm(req.agent, 'kb.manage')) throw new ApiError(403, 'Permission denied');
  const faq = await Faq.findOne({ _id: req.params.id, ...(req.companyId ? { company: req.companyId } : {}) });
  if (!faq) throw new ApiError(404, 'FAQ not found');
  await Faq.deleteOne({ _id: faq._id });
  await auditRequired({ company: req.companyId, actorType: 'agent', actor: req.agent._id, actorName: req.agent.name, action: 'knowledge.article_deleted', entityType: 'faq', entityId: faq._id, before: { question: faq.question, lifecycle: faq.lifecycle }, req });
...
exports.transitionFaq = asyncHandler(async (req, res) => {
  ...
  await faq.save();
  await auditRequired({ company: req.companyId, actorType: 'agent', actor: req.agent._id, actorName: req.agent.name, action: 'knowledge.article_transitioned', entityType: 'faq', entityId: faq._id, before: { lifecycle: from }, after: { lifecycle: faq.lifecycle, isPublished: faq.isPublished }, req });
  res.json({ success: true, faq });
});
