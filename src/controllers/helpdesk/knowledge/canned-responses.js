...
exports.listCanned = asyncHandler(async (req, res) => {
  const items = await CannedResponse.find(
    req.companyId ? { company: req.companyId } : {},
  ).sort({ title: 1 });
  res.json({ success: true, items });
});
exports.createCanned = asyncHandler(async (req, res) => {
  if (!hasPerm(req.agent, "canned.manage"))
    throw new ApiError(403, "Permission denied");
  const { title, response } = req.body;
  if (!title || !response)
    throw new ApiError(422, "Title and response are required");
...
exports.updateCanned = asyncHandler(async (req, res) => {
  ...
  if (req.companyId && String(canned.company) !== String(req.companyId))
...
exports.renderCanned = asyncHandler(async (req, res) => {
  ...
  if (
    req.companyId &&
    canned.company &&
    String(canned.company) !== String(req.companyId)
  ) {
    throw new ApiError(403, "Access denied");
  }
  ...
  if (ticketNumber) {
    ...
    data = {
      ...(await ticketService.buildTicketContext(ticket)),
  ...
  const withPercents = String(canned.response || "").replace(
    /\{\{\s*([\w.]+)\s*\}\}/g,
    "%{$1}",
  );
  ...
  res.json({
    success: true,
    title: canned.title,
    rendered: renderTemplate(withPercents, vars),
  });
});
