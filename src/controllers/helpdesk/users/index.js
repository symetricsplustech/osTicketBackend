...
exports.createUser = asyncHandler(async (req, res) => {
  if (!hasPerm(req.agent, "users.manage"))
...
exports.createOrg = asyncHandler(async (req, res) => {
  if (!hasPerm(req.agent, "orgs.manage"))
    throw new ApiError(403, "Permission denied");
  const { name, address, phone, website, domain, accountManager, notes } =
    req.body;
...
exports.getOrg = asyncHandler(async (req, res) => {
  ...
  if (req.companyId && String(org.company) !== String(req.companyId))
    throw new ApiError(403, "Access denied");
