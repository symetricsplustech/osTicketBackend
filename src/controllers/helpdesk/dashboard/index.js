...
exports.dashboard = asyncHandler(async (req, res) => {
  ...
  const base = scopeTicketQuery(agent, {
    status: { $ne: Ticket.STATUSES.DELETED },
  });
...
exports.queues = asyncHandler(async (req, res) => {
  ...
  const count = async (status) =>
    Ticket.countDocuments(status ? { ...base, status } : base);
  const [
    all,
    open,
    assigned,
    ...
    Ticket.countDocuments({
      ...
      status: {
        $in: [
          Ticket.STATUSES.PENDING_CUSTOMER,
          Ticket.STATUSES.PENDING_VENDOR,
          Ticket.STATUSES.PENDING_APPROVAL,
          Ticket.STATUSES.ON_HOLD,
        ],
      },
    }),
