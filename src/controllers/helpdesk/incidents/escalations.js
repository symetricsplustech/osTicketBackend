...
exports.listEscalations = asyncHandler(async (req, res) => {
  if (!canManageEscalations(req.agent))
    throw new ApiError(403, "Permission denied");
  const items = await EscalationRule.find(
...
exports.createEscalation = asyncHandler(async (req, res) => {
  ...
  if (!name || !String(name).trim())
    throw new ApiError(422, "Rule name is required");
  if (priority && !(await isValidPriority(priority)))
    throw new ApiError(422, "Invalid priority");
  if (
    action?.raisePriorityTo &&
    !(await isValidPriority(action.raisePriorityTo))
  )
    throw new ApiError(422, "Invalid raise priority");
...
exports.updateEscalation = asyncHandler(async (req, res) => {
  ...
  if (req.companyId && String(rule.company) !== String(req.companyId))
  ...
  if (overdueMinutes !== undefined)
    rule.overdueMinutes = parseInt(overdueMinutes, 10) || 0;
  if (priority !== undefined && !(await isValidPriority(priority)))
    throw new ApiError(422, "Invalid priority");
  if (action !== undefined) {
    rule.action = {
      raisePriorityTo: action.raisePriorityTo || null,
...
exports.deleteTicket = asyncHandler(async (req, res) => {
  if (!hasPerm(req.agent, "tickets.delete"))
    throw new ApiError(403, "Permission denied");
