...
exports.suggestTriage = asyncHandler(async (req, res) => {
  ...
  if (!text && !ticketNumber)
    throw new ApiError(422, "Provide subject/details or a ticketNumber");
  let queryText = text;
  ...
  res.json({
    ...
    note: docs.length
      ? undefined
      : "No resolved tickets yet — suggestions activate as history accumulates",
    ...result,
  });
});
...
exports.summarize = asyncHandler(async (req, res) => {
  ...
  if (ticketNumber) {
    ...
    entries = thread
      ...
      .filter((e) => e.body);
    if (!entries.length && t.body)
      entries = [{ author: "requester", type: "message", body: t.body }];
  } else if (incidentId) {
    ...
    title = inc.title || "";
    entries = [...(inc.timeline || []), ...(inc.updates || [])]
      .map((u) => ({
        author: u.by || "system",
        type: "update",
        body: u.message || u.status || "",
        createdAt: u.at,
      }))
      .filter((e) => e.body);
    if (inc.description || inc.summary)
      entries.unshift({
        author: "system",
        type: "update",
        body: inc.description || inc.summary,
      });
  } else if (text) {
...
exports.suggestRefs = asyncHandler(async (req, res) => {
  ...
  const [depts, topics] = await Promise.all([
    ...
    HelpTopic.find({
      status: "active",
      ...(req.companyId
        ? { $or: [{ company: req.companyId }, { company: null }] }
        : {}),
    })
      .select("topic name")
      .sort({ topic: 1 })
