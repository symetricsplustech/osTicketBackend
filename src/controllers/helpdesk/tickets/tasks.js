...
exports.addTask = asyncHandler(async (req, res) => {
  ...
  if (!String(title || '').trim()) throw new ApiError(422, 'Task title is required');
  const parsedDueDate = dueDate ? new Date(dueDate) : null;
  if (parsedDueDate && Number.isNaN(parsedDueDate.getTime())) throw new ApiError(422, 'Invalid task due date');
  if (assignedTo && !(await Agent.exists({ _id: assignedTo, company: ticket.company, isActive: true }))) {
    throw new ApiError(404, 'Assigned agent not found in this tenant');
  }
  ...
  await auditRequired({ company: ticket.company, actorType: 'agent', actor: req.agent._id, actorName: req.agent.name, action: 'ticket.task_created', entityType: 'ticket', entityId: ticket._id, after: { taskId: task._id, assignedTo: task.assignedTo, dueDate: task.dueDate }, req });
...
exports.updateTask = asyncHandler(async (req, res) => {
  ...
  const before = { title: task.title, description: task.description, assignedTo: task.assignedTo, dueDate: task.dueDate, status: task.status };
  if (assignedTo && !(await Agent.exists({ _id: assignedTo, company: ticket.company, isActive: true }))) {
    throw new ApiError(404, 'Assigned agent not found in this tenant');
  }
  if (title !== undefined && !String(title).trim()) throw new ApiError(422, 'Task title is required');
  if (dueDate !== undefined && dueDate && Number.isNaN(new Date(dueDate).getTime())) throw new ApiError(422, 'Invalid task due date');
  if (status !== undefined && !['open', 'closed'].includes(status)) throw new ApiError(422, 'Invalid task status');
  ...
  await task.save();
  await auditRequired({ company: ticket.company, actorType: 'agent', actor: req.agent._id, actorName: req.agent.name, action: 'ticket.task_updated', entityType: 'ticket', entityId: ticket._id, before, after: { title: task.title, description: task.description, assignedTo: task.assignedTo, dueDate: task.dueDate, status: task.status, taskId: task._id }, req });
