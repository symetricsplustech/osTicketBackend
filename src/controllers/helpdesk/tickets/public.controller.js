...
exports.myStatus = asyncHandler(async (req, res) => {
  ...
  if (!page)
    page = await StatusPage.findOne({ isPublic: true }).sort({ createdAt: 1 });
...
exports.chatPost = asyncHandler(async (req, res) => {
  ...
  let conversation = await require("../../../models/Conversation").findById(
    req.params.id,
  );
  ...
  const fallbackCompany =
    ...
    (await Company.findOne().sort({ createdAt: 1 }))?._id ||
    null;
  const ticketService = require("../../../services/ticket.service");
  let user = null;
  if (userId) user = await require("../../../models/User").findById(userId);
  else if (guestEmail) {
    user = await ticketService.findOrCreateUser({
  ...
  if (applyBotReply) {
    try {
      const {
        virtualAgent,
      } = require("../../../services/virtualAgent.service");
...
exports.submitCsat = asyncHandler(async (req, res) => {
  ...
  if (!ticketNumber || rating == null)
    throw new ApiError(422, "ticketNumber and rating required");
  if (!req.user || req.agent)
    throw new ApiError(403, "Requester access required");
  const ticket = await Ticket.findOne({
    number: String(ticketNumber).toUpperCase(),
...
exports.formConfig = asyncHandler(async (req, res) => {
  ...
  const company = req.query.company
    ? await Company.findById(req.query.company).select(
        "name supportEmail email domain",
      )
    : await Company.findOne()
        .sort({ createdAt: 1 })
        .select("name supportEmail email domain");
...
exports.serviceCatalog = asyncHandler(async (req, res) => {
  ...
  if (!company)
    throw new ApiError(422, "Tenant-authenticated catalog access is required");
  const items = await ServiceCatalogItem.find({
    company,
...
exports.surveysForTicket = asyncHandler(async (req, res) => {
  ...
  if (!req.user || req.agent)
    throw new ApiError(403, "Requester access required");
  const ticket = await Ticket.findOne({
    number: String(ticketNumber).trim().toUpperCase(),
    company: req.companyId,
  });
  if (!ticket) throw new ApiError(404, "Ticket not found");
  if (String(ticket.user) !== String(req.user._id))
  ...
  const surveys = await Survey.find({
    company: ticket.company,
    isActive: true,
  }).select("name type question scale");
  res.json({
    success: true,
    items: surveys,
...
exports.registerCompany = asyncHandler(async (req, res) => {
  const { name, email, domain, ownerName, ownerEmail, ownerPassword } =
    req.body;
  if (!name || !ownerName || !ownerEmail || !ownerPassword) {
    throw new ApiError(
      422,
      "Company name, owner name, owner email and password are required",
    );
  }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(ownerEmail)))
    throw new ApiError(422, "Valid owner email is required");
  const Company = require("../../../models/Company");
  ...
  if (recent >= 3)
    throw new ApiError(
      429,
      "Too many registrations from this email, try again tomorrow",
    );
  ...
  res
    ...
    .json({
      success: true,
      message:
        "Company registered — check the owner inbox to verify and activate.",
...
exports.verifyCompany = asyncHandler(async (req, res) => {
  ...
  await Agent.updateMany(
    { company: company._id, isAdmin: true },
    { $set: { isActive: true } },
  );
  return done(
    true,
    "Company verified",
    `“${company.name}” is now active on a 14-day trial. You can sign in to the admin portal.`,
  );
});
