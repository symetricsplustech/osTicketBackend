...
exports.listSavedQueues = asyncHandler(async (req, res) => {
  ...
  const items = await SavedQueue.find({ agent: req.agent._id }).sort({
    name: 1,
  });
...
exports.createSavedQueue = asyncHandler(async (req, res) => {
  ...
  if (!name || !String(name).trim())
    throw new ApiError(422, "Queue name is required");
...
exports.exportTickets = asyncHandler(async (req, res) => {
  if (!hasPerm(req.agent, "tickets.view"))
  ...
  if (statusKey && statusKey !== "all" && statusKey !== "active")
  ...
  const tickets = await require("../../../models/helpdesk/tickets/Ticket")
    .find(query)
    .sort({ updatedAt: -1 })
    .limit(5000)
  ...
  const rows = tickets.map((t) =>
    [
      ...
      t.lastActivity ? new Date(t.lastActivity).toISOString() : "",
