...
exports.openForm = asyncHandler(async (req, res) => {
  const companyId =
    req.companyId ||
    (req.query.company && req.query.company !== "null"
      ? req.query.company
      : null);
  ...
  const fieldQuery = companyId
    ? { isActive: true, $or: [{ company: companyId }, { company: null }] }
    : { isActive: true };
  const formQuery = companyId
    ? { isActive: true, $or: [{ company: companyId }, { company: null }] }
    : { isActive: true };
  const [topics, departments, settings, customFields, forms, priorities] =
    await Promise.all([
      HelpTopic.find(topicQuery)
        .sort({ topic: 1 })
        .populate("department", "name"),
      Department.find(deptQuery).sort({ name: 1 }),
      SystemSetting.getSettings(),
      CustomField.find(fieldQuery)
        .sort({ sortOrder: 1 })
        .populate("helpTopic", "topic"),
      TicketForm.find(formQuery)
        .sort({ name: 1 })
        .populate("helpTopic", "topic")
        .populate("fields"),
      (async () => {
  ...
  const emailToTicket =
    settings.system?.emailToTicket || config.email.emailToTicket || "";
  ...
  try {
    if (companyId) {
      ...
      const co = await Company.findById(companyId).select(
        "name email supportEmail domain",
      );
      if (co) {
...
exports.create = asyncHandler(async (req, res) => {
  const { subject, details, topic, priority, impact, urgency, customData } =
    req.body;
  if (!String(subject || "").trim() || !String(details || "").trim())
    throw new ApiError(422, "Subject and details are required");
  let parsedCustom = {};
  if (customData) {
    try {
      parsedCustom =
  ...
  if (priority) {
    ...
    if (!(await isValidPriority(priority, companyId)))
      throw new ApiError(422, "Invalid ticket priority");
  }
...
exports.checkTicketStatus = asyncHandler(async (req, res) => {
  ...
  if (!req.user || req.agent)
    throw new ApiError(403, "Requester access required");
  const ticketQuery = {
...
const loadTicketWithAccess = async (req, { requirePermission = null } = {}) => {
  ...
  if (
    req.companyId &&
    ticket.company &&
    String(ticket.company) !== String(req.companyId)
  ) {
    throw new ApiError(403, "You do not have access to this ticket");
  }
...
exports.viewTicket = asyncHandler(async (req, res) => {
  const ticket = await loadTicketWithAccess(req, {
    requirePermission: USER_PERMISSIONS.TICKET_VIEW,
  });
...
exports.reply = asyncHandler(async (req, res) => {
  const ticket = await loadTicketWithAccess(req, {
    requirePermission: USER_PERMISSIONS.TICKET_REPLY,
  });
  ...
  if (
    ...
    if (!ticketService.reopenWindowAllows(ticket, replySettings)) {
      ...
      return res
        ...
        .json({
          success: true,
          message: `Previous ticket ${ticket.number} is past the reply window — created follow-up ${created.number}`,
          ticket: created,
          followUp: true,
  ...
  require("../../../services/audit.service").audit({
    company: ticket.company || req.companyId || null,
    actorType: "user",
    actor: req.user._id,
  ...
  await notifyOwnerOfEmployeeAction({
    ticket,
    user: req.user,
    type: "reply",
    action: "replied",
  ...
  const agent = ticket.agent
    ? await require("../../../models/Agent").findById(ticket.agent)
    : null;
...
exports.reopenTicket = asyncHandler(async (req, res) => {
  ...
  if (!ticketService.reopenWindowAllows(ticket, settings)) {
    throw new ApiError(
      400,
      "Reopen window expired — please open a follow-up ticket instead",
    );
  }
...
exports.mergeTickets = asyncHandler(async (req, res) => {
  ...
  if (sourceTicket.user._id.equals(targetTicket.user._id)) {
    throw new ApiError(
      400,
      "Cannot merge a ticket into itself or a ticket in the same user account",
    );
  }
...
exports.referTicket = asyncHandler(async (req, res) => {
  ...
  if (!targetUserId && !targetTeamId && !targetDeptId) {
    throw new ApiError(
      400,
      "At least one target (user, team, or department) is required",
    );
  }
...
exports.deleteTicket = asyncHandler(async (req, res) => {
  const ticket = await loadTicketWithAccess(req, {
    requirePermission: USER_PERMISSIONS.TICKET_DELETE,
  });
  ...
  await ticketService.addSystemEvent({
    ticket,
    message: "Ticket deleted by user",
  });
  await notifyOwnerOfEmployeeAction({
    ticket,
    user: req.user,
...
exports.orgApprovals = asyncHandler(async (req, res) => {
  ...
  const items = await approvalFlow.listPendingOrgApprovals({
    user: req.user,
...
exports.decideTicketApproval = asyncHandler(async (req, res) => {
  ...
  const result = await approvalFlow.decideCustomerApproval({
    ticket,
    user: req.user,
    decision,
    note,
