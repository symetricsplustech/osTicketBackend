...
exports.mergeTickets = asyncHandler(async (req, res) => {
  ...
  const target = await loadTicketForAgent(
    String(targetNumber).toUpperCase(),
    req.agent,
  );
  ...
  await TicketThreadModel.updateMany(
    { ticket: source._id },
    { $set: { ticket: target._id } },
  );
  const collabs = new Set([
    ...(target.collaborators || []).map((c) => String(c)),
    ...(source.collaborators || []).map((c) => String(c)),
  ]);
  target.collaborators = [...collabs].map((c) =>
    require("mongoose").Types.ObjectId(c),
  );
  await target.save();
  ...
  await ticketService.addSystemEvent({
    ticket: target,
    message: `Ticket ${source.number} merged into this ticket by ${req.agent.name}`,
  });
  await ticketService.addSystemEvent({
    ticket: source,
    message: `Ticket merged into ${target.number} by ${req.agent.name}`,
  });
  ...
  const meta = {
    ...
    entityId: source._id,
    before: prev,
    after: { number: target.number, status: source.status },
    req,
  };
  await audit({ ...meta });
  await audit({ ...meta, after: { mergedFrom: source.number } });
  const bus = require("../../../services/events");
  bus.emit("ticket.merged", {
    company: source.company,
    ticketId: target._id,
    ticketNumber: target.number,
    mergedFrom: source.number,
    actor: req.agent._id,
  });
  res.json({
    success: true,
    message: `Ticket ${source.number} merged into ${target.number}`,
    ticket: target,
  });
});
exports.splitTicket = asyncHandler(async (req, res) => {
  ...
  if (!Array.isArray(threadIds) || !threadIds.length)
    throw new ApiError(422, "Select at least one thread to split out");
  if (!subject || !String(subject).trim())
    throw new ApiError(422, "Subject is required for the new ticket");
  const TicketThreadModel = require("../../../models/helpdesk/tickets/TicketThread");
  const TicketLink = require("../../../models/helpdesk/tickets/TicketLink");
  const threads = await TicketThreadModel.find({
    _id: { $in: threadIds },
    ticket: source._id,
    deletedAt: null,
    type: "message",
  ...
  if (!threads.length)
    throw new ApiError(404, "No matching message threads to split");
  ...
  await ticketService.addSystemEvent({
    ticket: source,
    message: `${threads.length} message(s) split into new ticket ${newTicket.number} by ${req.agent.name}`,
  });
  await ticketService.addSystemEvent({
    ticket: newTicket,
    message: `Ticket split from ${source.number} by ${req.agent.name}`,
  });
  const audit = auditRequired;
  await audit({
    company: source.company,
    ...
    entityType: "ticket",
    entityId: source._id,
    after: { splitInto: newTicket.number, threads: threads.length },
    req,
  });
  const bus = require("../../../services/events");
  ...
  res
    ...
    .json({
      success: true,
      message: `Ticket ${newTicket.number} created from split`,
      ticket: newTicket,
    });
});
exports.updateThread = asyncHandler(async (req, res) => {
  ...
  if (!body || !String(body).trim())
    throw new ApiError(422, "Message body is required");
  const TicketThreadModel = require("../../../models/helpdesk/tickets/TicketThread");
  const thread = await TicketThreadModel.findOne({
    _id: req.params.threadId,
    ticket: ticket._id,
    deletedAt: null,
  });
  if (!thread) throw new ApiError(404, "Thread entry not found");
  if (thread.posterType === "system")
    throw new ApiError(400, "System events cannot be edited");
  thread.editHistory = thread.editHistory || [];
  thread.editHistory.push({
    at: new Date(),
    by: req.agent._id,
    preview: String(thread.body).slice(0, 120),
  });
...
exports.deleteThread = asyncHandler(async (req, res) => {
  ...
  if (!thread) throw new ApiError(404, "Thread entry not found");
  if (thread.posterType === "system")
    throw new ApiError(400, "System events cannot be deleted");
  ...
  await audit({
    ...
    entityType: "ticket",
    entityId: ticket._id,
    after: { threadId: thread._id, type: thread.type },
    req,
  });
  const threads = await TicketThreadModel.find({
    ticket: ticket._id,
...
exports.pauseSla = asyncHandler(async (req, res) => {
  ...
  await audit({
    ...
    entityType: "ticket",
    entityId: ticket._id,
    after: { dueDate: updated.dueDate, waitingOn: updated.waitingOn },
    req,
  });
