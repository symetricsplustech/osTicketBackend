...
exports.reply = asyncHandler(async (req, res) => {
  ...
  await ticketService.addThreadEntry({
    ...
    body: message,
    attachments,
    auditContext: { actorType: 'agent', actorId: req.agent._id, actorName: req.agent.name, req },
  });
  await ticketService.addSystemEvent({ ticket, message: `Response posted by ${req.agent.name}` });
  await auditRequired({ company: ticket.company, actorType: 'agent', actor: req.agent._id, actorName: req.agent.name, action: 'ticket.replied', entityType: 'ticket', entityId: ticket._id, after: { attachmentCount: attachments.length, hasAttachments: attachments.length > 0 }, req });
...
exports.addNote = asyncHandler(async (req, res) => {
  ...
  await auditRequired({ company: ticket.company, actorType: 'agent', actor: req.agent._id, actorName: req.agent.name, action: 'ticket.note_added', entityType: 'ticket', entityId: ticket._id, after: { note: true }, req });
...
exports.assign = asyncHandler(async (req, res) => {
  ...
  await auditRequired({ company: ticket.company, actorType: 'agent', actor: req.agent._id, actorName: req.agent.name, action: 'ticket.assigned', entityType: 'ticket', entityId: ticket._id, after: { agentId: assignedTo?._id || null, teamId: assignedTeam?._id || null }, req });
...
exports.transfer = asyncHandler(async (req, res) => {
  ...
  await auditRequired({ company: ticket.company, actorType: 'agent', actor: req.agent._id, actorName: req.agent.name, action: 'ticket.transferred', entityType: 'ticket', entityId: ticket._id, after: { deptId: dept._id, deptName: dept.name }, req });
...
exports.lockTicket = asyncHandler(async (req, res) => {
  ...
  await ticket.save();
  await auditRequired({ company: ticket.company, actorType: 'agent', actor: req.agent._id, actorName: req.agent.name, action: 'ticket.locked', entityType: 'ticket', entityId: ticket._id, after: { lockedBy: req.agent._id, lockExpiresAt: ticket.lockExpiresAt }, req });
...
exports.unlockTicket = asyncHandler(async (req, res) => {
  ...
  ticket.lockExpiresAt = null;
  await ticket.save();
  await auditRequired({ company: ticket.company, actorType: 'agent', actor: req.agent._id, actorName: req.agent.name, action: 'ticket.unlocked', entityType: 'ticket', entityId: ticket._id, after: { lockedBy: null, lockExpiresAt: null }, req });
...
exports.updateFields = asyncHandler(async (req, res) => {
  ...
  const before = { subject: ticket.subject, priority: ticket.priority, dueDate: ticket.dueDate, source: ticket.source, asset: ticket.asset };
  const changed = [];
  ...
  if (dueDate !== undefined) {
    ...
    if (parsedDueDate && Number.isNaN(parsedDueDate.getTime())) throw new ApiError(422, 'Invalid due date');
  ...
  await auditRequired({ company: ticket.company, actorType: 'agent', actor: req.agent._id, actorName: req.agent.name, action: 'ticket.fields_updated', entityType: 'ticket', entityId: ticket._id, before, after: { subject: ticket.subject, priority: ticket.priority, dueDate: ticket.dueDate, source: ticket.source, asset: ticket.asset }, req });
...
exports.claim = asyncHandler(async (req, res) => {
  ...
  await auditRequired({ company: ticket.company, actorType: 'agent', actor: req.agent._id, actorName: req.agent.name, action: 'ticket.claimed', entityType: 'ticket', entityId: ticket._id, after: { agentId: req.agent._id, claimed }, req });
...
exports.addCollaborator = asyncHandler(async (req, res) => {
  ...
  await auditRequired({ company: ticket.company, actorType: 'agent', actor: req.agent._id, actorName: req.agent.name, action: 'ticket.collaborator_added', entityType: 'ticket', entityId: ticket._id, after: { collaboratorId: user._id }, req });
  await notifyUser({
    userId: user._id,
    company: ticket.company,
    type: 'collaborator',
...
exports.removeCollaborator = asyncHandler(async (req, res) => {
  const ticket = await loadTicketForAgent(req.params.number, req.agent);
  const existing = (ticket.collaborators || []).some((c) => c && String(c._id || c) === String(req.params.userId));
  if (!existing) throw new ApiError(404, 'Collaborator not found on this ticket');
  await Ticket.updateOne({ _id: ticket._id }, { $pull: { collaborators: req.params.userId } });
  await ticketService.addSystemEvent({
    ticket,
    message: `Collaborator removed by ${req.agent.name}`,
  });
  await auditRequired({ company: ticket.company, actorType: 'agent', actor: req.agent._id, actorName: req.agent.name, action: 'ticket.collaborator_removed', entityType: 'ticket', entityId: ticket._id, before: { collaboratorId: req.params.userId }, after: { collaboratorId: null }, req });
  const updated = await Ticket.findById(ticket._id).populate('collaborators', 'name email');
