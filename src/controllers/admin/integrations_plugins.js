  ...
  create: asyncHandler(async (req, res) => {
    ...
    if (body.config && typeof body.config === "string") {
      try {
        ...
        throw new ApiError(422, "Invalid config JSON");
      }
    }
...
exports.importTickets = asyncHandler(async (req, res) => {
  ...
  const idx = (k) => {
    if (!hasHeader)
      return { subject: 0, email: 1, status: 2, priority: 3, department: 4 }[k];
  ...
  for (const r of dataRows) {
    ...
    if (deptName) {
      if (!deptCache[deptName]) {
        deptCache[deptName] = await Department.findOne({
          name: deptName,
    ...
    if (!["open", "pending", "hold", "closed", "deleted"].includes(status)) {
      skipped++;
      continue;
    }
