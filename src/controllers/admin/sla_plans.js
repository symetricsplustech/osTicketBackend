...
exports.listSlaPlans = asyncHandler(async (req, res) => {
  const plans = await SlaPlan.find(
  ...
  const plan = await SlaPlan.create({
    ...
    targets: {
      ...
      ...(responseMinutes !== undefined
        ? { first_response: Number(responseMinutes) / 60 }
        : {}),
      ...(resolutionMinutes !== undefined
        ? { resolution: Number(resolutionMinutes) / 60 }
        : {}),
    },
...
exports.updateSlaPlan = asyncHandler(async (req, res) => {
  ...
  if (req.companyId && String(plan.company) !== String(req.companyId))
    throw new ApiError(403, "Access denied");
  const {
    name,
    gracePeriod,
  ...
  if (targets !== undefined)
    plan.targets = { ...plan.targets?.toObject?.(), ...targets };
  if (responseMinutes !== undefined)
    plan.targets.first_response = Number(responseMinutes) / 60;
  if (resolutionMinutes !== undefined)
    plan.targets.resolution = Number(resolutionMinutes) / 60;
...
exports.slaDashboard = asyncHandler(async (req, res) => {
  ...
  const since = new Date(
    Date.now() - Math.min(Number(req.query.days) || 30, 365) * 86400000,
  );
  const [events, openTickets, atRisk, breached, departments] =
    await Promise.all([
      SlaEvent.find({
        level: "tenant",
        company: req.companyId,
        occurredAt: { $gte: since },
        event: { $in: ["met", "breached"] },
      }).lean(),
      ...
      Ticket.countDocuments({
        company: req.companyId,
        isOverdue: false,
        dueDate: { $gt: new Date(), $lte: new Date(Date.now() + 2 * 3600000) },
      }),
      Ticket.countDocuments({
        company: req.companyId,
        $or: [{ isOverdue: true }, { responseBreached: true }],
      }),
      Ticket.aggregate([
  ...
  const summarize = (clock) => {
    ...
    return {
      ...
      met,
      breached: selected.length - met,
      compliance: selected.length ? (met / selected.length) * 100 : 100,
    };
  };
  res.json({
    success: true,
    data: {
      openTickets,
      withinSla: Math.max(0, openTickets - breached),
      atRisk,
      breached,
      response: summarize("first_response"),
...
exports.ticketSlaHistory = asyncHandler(async (req, res) => {
  ...
  const ticket = await Ticket.findOne({
    ...
    .select("_id number sla responseDueAt resolutionDueAt")
    .populate("sla", "name");
