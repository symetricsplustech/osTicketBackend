...
exports.exportUsers = asyncHandler(async (req, res) => {
  ...
  const users = await User.find(comp)
...
exports.exportOrgs = asyncHandler(async (req, res) => {
  const comp = req.companyId ? { company: req.companyId } : {};
  const orgs = await Organization.find(comp)
    .populate("accountManager", "name")
    .sort({ name: 1 })
    .lean();
  csvDownload(
    res,
    `orgs-${new Date().toISOString().slice(0, 10)}`,
    [
      "name",
      "address",
      "phone",
...
exports.exportAgents = asyncHandler(async (req, res) => {
  ...
  csvDownload(
    ...
    agents.map((a) => [
      ...
      a.email,
      a.role?.name || "",
      (a.departments || []).map((d) => d.department?.name).join("; "),
      a.isAdmin ? "yes" : "no",
      a.isActive ? "yes" : "no",
...
exports.exportTickets = asyncHandler(async (req, res) => {
  const comp = req.companyId ? { company: req.companyId } : {};
  const tickets = await Ticket.find(comp)
    .sort({ updatedAt: -1 })
    .limit(5000)
...
exports.importUsers = asyncHandler(async (req, res) => {
  ...
  for (const r of dataRows) {
    ...
    if (orgName) {
      if (!orgCache[orgName]) {
        orgCache[orgName] = await Organization.findOne({
          name: orgName,
...
exports.importOrgs = asyncHandler(async (req, res) => {
  ...
  const idx = (k) => {
    if (!hasHeader)
      return { name: 0, address: 1, phone: 2, website: 3, domain: 4 }[k];
    return header.indexOf(k);
  };
  let created = 0,
    skipped = 0;
