...
exports.createUser = asyncHandler(async (req, res) => {
  ...
  let orgId = null;
  if (organization) {
    if (!require('mongoose').isValidObjectId(organization)) throw new ApiError(422, 'Invalid organization selected');
    const org = await Organization.findOne({ _id: organization, ...(req.companyId ? { company: req.companyId } : {}) });
    if (!org) throw new ApiError(422, 'Organization not found');
    orgId = org._id;
  }
...
exports.updateUser = asyncHandler(async (req, res) => {
  ...
  if (organization !== undefined) {
    ...
    } else if (!require('mongoose').isValidObjectId(organization)) {
      throw new ApiError(422, 'Invalid organization selected');
    } else {
