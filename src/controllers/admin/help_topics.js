...
exports.createHelpTopic = asyncHandler(async (req, res) => {
  ...
  const ht = await HelpTopic.create({
    ...
    autoresponder:
      autoresponder && autoresponder.enabled !== undefined
        ? autoresponder
        : undefined,
...
exports.updateHelpTopic = asyncHandler(async (req, res) => {
  ...
  if (req.companyId && String(ht.company) !== String(req.companyId))
    throw new ApiError(403, "Access denied");
  const {
    topic,
    category,
  ...
  if (autoAssignAgent !== undefined)
    ht.autoAssignAgent = autoAssignAgent || null;
