...
exports.listEmailTemplates = asyncHandler(async (req, res) => {
  ...
  res.json({
    ...
    items,
    meta: {
      triggers: EmailTemplate.TRIGGERS,
...
exports.getEmailTemplate = asyncHandler(async (req, res) => {
  ...
  if (
    req.companyId &&
    template.company &&
    String(template.company) !== String(req.companyId)
  ) {
    throw new ApiError(403, "Access denied");
  }
  res.json({
...
exports.createEmailTemplate = asyncHandler(async (req, res) => {
  ...
  if (!key)
    throw new ApiError(
      422,
      "Trigger (when the notification is sent) is required",
    );
  ...
  if (existing) {
    throw new ApiError(
      409,
      `A template for this trigger already exists${companyId ? " for your company" : ""}`,
    );
  }
...
exports.updateEmailTemplate = asyncHandler(async (req, res) => {
  ...
  if (companyId && !template.company) {
    let override = await EmailTemplate.findOne({
      key: template.key,
      company: companyId,
    });
    if (override) {
    ...
    override = await EmailTemplate.create({
      ...
      recipient:
        template.recipient ||
        EmailTemplate.getTriggerByKey(template.key).recipient,
    ...
    return res
      .status(201)
      .json({ success: true, template: override, overridden: true });
  }
...
exports.deleteEmailTemplate = asyncHandler(async (req, res) => {
  ...
  if (
    companyId &&
    (!template.company || String(template.company) !== String(companyId))
  ) {
    throw new ApiError(
      403,
      "System templates cannot be deleted. Edit them to create a company-specific override instead.",
    );
  }
