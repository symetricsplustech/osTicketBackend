...
exports.listTeams = asyncHandler(async (req, res) => {
  ...
  const teams = await Team.find(comp)
    .populate("lead", "name")
...
exports.createTeam = asyncHandler(async (req, res) => {
  ...
  const team = await Team.create({
    name,
    lead: lead || null,
    leadTitle: (leadTitle || "Team Lead").trim(),
    members: members || [],
    notes: notes || "",
...
exports.updateTeam = asyncHandler(async (req, res) => {
  ...
  if (req.companyId && String(team.company) !== String(req.companyId))
    throw new ApiError(403, "Access denied");
  const { name, lead, leadTitle, members, notes, status } = req.body;
  if (name) team.name = name;
  if (lead !== undefined) team.lead = lead || null;
  if (leadTitle !== undefined)
    team.leadTitle = String(leadTitle || "Team Lead").trim();
