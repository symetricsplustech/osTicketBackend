...
exports.updateFaqCategory = asyncHandler(async (req, res) => {
  ...
  if (req.companyId && String(cat.company) !== String(req.companyId))
    throw new ApiError(403, "Access denied");
...
exports.createFaq = asyncHandler(async (req, res) => {
  ...
  if (visibility !== undefined && !VALID_VISIBILITY.includes(visibility))
    throw new ApiError(422, "Invalid visibility");
  ...
  const targetLifecycle =
    lifecycle || (isPublished === false ? "draft" : "published");
  if (!VALID_LIFECYCLE.includes(targetLifecycle))
    throw new ApiError(422, "Invalid lifecycle state");
...
exports.updateFaq = asyncHandler(async (req, res) => {
  ...
  if (visibleDepartments !== undefined)
    faq.visibleDepartments = visibleDepartments;
  ...
  if (lifecycle !== undefined) {
    ...
    if (isPublished && ["draft", "archived", "expired"].includes(faq.lifecycle))
