...
exports.listAgents = asyncHandler(async (req, res) => {
  ...
  const agents = await Agent.find(comp)
    .populate("role", "name")
...
exports.createAgent = asyncHandler(async (req, res) => {
  ...
  if (level !== undefined && !["L1", "L2", "L3"].includes(level))
    throw new ApiError(422, "Invalid agent level");
...
exports.updateAgent = asyncHandler(async (req, res) => {
  ...
  if (req.companyId && String(agent.company) !== String(req.companyId))
    throw new ApiError(403, "Access denied");
  const {
    name,
    email,
  ...
  if (permissions !== undefined)
    agent.permissions = Array.isArray(permissions) ? permissions : [];
...
exports.createRole = asyncHandler(async (req, res) => {
  ...
  if (category === "platform") {
    auditRoleChange(req, "role.platform_creation_denied", {
      name,
      category,
      scope: "platform",
    });
    throw new ApiError(
      422,
      "Platform roles can only be managed through the platform administration boundary",
    );
  }
...
exports.updateRole = asyncHandler(async (req, res) => {
  ...
  if (role.scope !== "tenant" || String(role.company) !== String(req.companyId))
    throw new ApiError(403, "Access denied");
  ...
  if (category === "platform") {
    auditRoleChange(
      req,
      "role.platform_update_denied",
      { _id: role._id, name: role.name, category, scope: "platform" },
      before,
    );
    throw new ApiError(
      422,
