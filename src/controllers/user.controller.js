...
const ensureMainCustomer = (req) => {
  if (req.user.createdBy)
    throw new ApiError(403, "Only the account owner can manage employees");
};
...
exports.listEmployees = asyncHandler(async (req, res) => {
  ...
  const employees = await User.find({ createdBy: req.user._id }).sort({
    createdAt: -1,
  });
...
exports.markNotificationRead = asyncHandler(async (req, res) => {
  await Notification.updateOne(
    { _id: req.params.id, recipientType: "user", recipient: req.user._id },
    { $set: { read: true } },
  );
  res.json({ success: true });
});
...
exports.permissionsMeta = asyncHandler(async (req, res) => {
  res.json({
    success: true,
    permissions: USER_PERMISSION_LIST.map((key) => ({
      key,
      label: USER_PERMISSION_LABELS[key] || key,
    })),
  });
});
...
exports.updateSupportInbox = asyncHandler(async (req, res) => {
  ...
  const isAgentAdmin = !!(
    req.agent &&
    (req.agent.isAdmin || req.agent.role?.isAdmin)
  );
  const isOwner = !!(req.user && !req.user.createdBy && !req.agent);
  if (!isAgentAdmin && !isOwner) {
    throw new ApiError(
      403,
      "Only the company owner can configure the support inbox",
    );
  }
  const { supportEmail, domain } = req.body;
  const inbox = String(supportEmail || "")
    .toLowerCase()
    .trim();
  if (!EMAIL_RE.test(inbox))
    throw new ApiError(422, "Valid supportEmail is required");
  if (
    domain !== undefined &&
    String(domain).trim() &&
    !/^[a-z0-9.-]+\.[a-z]{2,}$/i.test(String(domain).trim())
  ) {
...
exports.requestEmailChange = asyncHandler(async (req, res) => {
  if (req.agent)
    throw new ApiError(
      403,
      "Agents update email via the agent profile, not here",
    );
  ...
  if (next === req.user.email)
    throw new ApiError(422, "This is already your support email");
  ...
  try {
    ...
    /* non-blocking: verification stays usable via token */
  }
  res.json({
    success: true,
    message:
      "Verification sent to your new address. Confirm within 30 minutes.",
    pendingEmail: next,
...
exports.confirmEmailChange = asyncHandler(async (req, res) => {
  ...
  if (taken)
    throw new ApiError(409, "This email was just taken by another account");
  user.email = next;
  ...
  res.json({
    success: true,
    message:
      "Support email updated. Mail from the new address to create tickets.",
    user,
  });
});
