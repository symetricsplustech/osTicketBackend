...
customRoleSchema.pre('validate', function blockSaasEscalation(next) {
  const banned = (this.permissions || []).filter((p) => typeof p === 'string' && /^(saas\.|platform\.|superadmin)/.test(p));
  if (banned.length) return next(new Error(`Custom roles cannot grant SaaS permissions: ${banned.join(', ')}`));
  next();
});
