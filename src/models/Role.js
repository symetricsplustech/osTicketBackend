...
roleSchema.pre("validate", function validateRoleBoundary(next) {
  if (this.scope === "platform") {
    if (this.company)
      return next(new Error("Platform roles cannot belong to a tenant"));
    if (this.category !== "platform")
      return next(new Error("Platform roles must use the platform category"));
  } else {
    if (!this.company)
      return next(new Error("Tenant roles require a tenant company"));
    if (this.category === "platform")
      return next(new Error("Tenant roles cannot use the platform category"));
  }
  next();
});
