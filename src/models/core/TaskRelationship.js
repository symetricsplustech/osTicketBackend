...
const taskRelationshipSchema = new mongoose.Schema(
  {
    ...
    targetTaskId: { type: mongoose.Schema.Types.ObjectId, ref: 'Task', required: true, index: true },
    relationshipType: {
      type: String,
      enum: ['relates_to', 'blocks', 'blocked_by', 'duplicates', 'caused_by', 'child_of', 'parent_of'],
      required: true,
    },
...
taskRelationshipSchema.index({ tenantId: 1, sourceTaskId: 1, targetTaskId: 1, relationshipType: 1 }, { unique: true });
