...
const WalkupInteractionSchema = new Schema({
  ...
  checkinId: { type: Schema.Types.ObjectId, ref: 'WalkupCheckin', required: true, index: true },
  appointmentId: { type: Schema.Types.ObjectId, ref: 'Appointment' },
  locationId: { type: Schema.Types.ObjectId, ref: 'WalkupLocation', required: true },
  userId: { type: Schema.Types.ObjectId, ref: 'User', required: true, index: true },
  technicianId: { type: Schema.Types.ObjectId, ref: 'User', required: true, index: true },
  type: { type: String, enum: ['checkin', 'consultation', 'troubleshooting', 'hardware_swap', 'software_install', 'access_request', 'training', 'other'], default: 'consultation' },
  subject: { type: String, required: true, trim: true },
  description: { type: String, trim: true, default: '' },
  resolution: { type: String, trim: true, default: '' },
  assetsInvolved: [{ type: Schema.Types.ObjectId, ref: 'Asset' }],
  assetsProvided: [{ assetId: { type: Schema.Types.ObjectId, ref: 'Asset' }, serialNumber: { type: String }, condition: { type: String, enum: ['new', 'refurbished', 'used'] } }],
  softwareInvolved: [{ type: String }],
  startedAt: { type: Date, default: Date.now },
  endedAt: { type: Date },
  durationMinutes: { type: Number, default: 0 },
  outcome: { type: String, enum: ['resolved', 'escalated', 'deferred', 'partial', 'no_action'], default: 'resolved' },
