...
const BUNDLES = {
  ...
  "users.manage": [
    ...
    "tenant.user.invite",
    "tenant.user.suspend",
    "tenant.user.reactivate",
    "tenant.user.deactivate",
    "tenant.user.archive",
    "tenant.user.restore",
    "tenant.user.reset_mfa",
    "tenant.user.revoke_sessions",
  ],
  "roles.manage": [
    "tenant.role.create",
    "tenant.role.read",
...
function expandGrants(rawKeys) {
  ...
  for (const raw of rawKeys || []) {
    ...
    if (REVERSE_ALIAS[raw])
      for (const legacy of REVERSE_ALIAS[raw]) out.add(legacy);
    if (REVERSE_BUNDLE[raw])
      for (const legacy of REVERSE_BUNDLE[raw]) out.add(legacy);
...
function validate() {
  const missing = [];
  for (const [legacy, canonical] of Object.entries(ALIAS)) {
    if (!isCanonicalKey(canonical))
      missing.push(`ALIAS ${legacy} -> ${canonical}`);
  }
  for (const [legacy, cannons] of Object.entries(BUNDLES)) {
  ...
  if (missing.length) {
    throw new Error(
      `legacyMappings validation failed:\n  ${missing.join("\n  ")}`,
    );
  }
}
validate();
