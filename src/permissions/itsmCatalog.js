...
const MODULES = [
  { key: 'core', namespace: 'itsm.core.*', label: 'Core Platform / Task Engine', moduleKey: 'helpdesk', scope: 'tenant' },
  { key: 'incident', namespace: 'itsm.incident.*', label: 'Incident Management', moduleKey: 'helpdesk', scope: 'tenant' },
  { key: 'problem', namespace: 'itsm.problem.*', label: 'Problem Management', moduleKey: 'helpdesk', scope: 'tenant' },
  { key: 'change', namespace: 'itsm.change.*', label: 'Change Management', moduleKey: 'helpdesk', scope: 'tenant' },
  { key: 'request_catalog', namespace: 'itsm.request_catalog.*', label: 'Request Management + Service Catalog', moduleKey: 'helpdesk', scope: 'tenant' },
...
const CRUD_ACTIONS = new Set(['create', 'read', 'update', 'delete', 'archive', 'restore', 'view_history', 'view_audit', 'export']);

// Patterns that mark a permission as high-risk for UI warnings + always-audit.
const HIGH_RISK_PATTERNS = [
  'delete', 'override', 'export', 'view_audit', 'impersonate', 'reset_mfa',
  'revoke_sessions', 'bulk_', 'restore', 'manage_access', 'security', 'break_glass',
  'demote', 'approve', 'reject', 'impersonation', 'privileged',
];
...
function deriveGroup(key) {
  ...
  if (rest.includes('screen') || rest.includes('ui_') || rest.includes('_page') || rest.includes('_list')) return 'screen';
      ...
      .map(([gkey, gkeys]) => {
        ...
        for (const k of gkeys) {
          ...
          if (gkey === 'automation') {
            ...
            res = rest[0] && CRUD_ACTIONS.has(rest[rest.length - 1]) ? rest[0] : (rest[0] || mod.key);
          }
          resources[res] = resources[res] || [];
          resources[res].push(k);
        }
        const resourceList = Object.entries(resources).map(([rkey, actions]) => ({
          key: rkey,
          label: humanize(rkey),
        ...
        return { key: gkey, label: gl ? gl.label : humanize(gkey), resources: resourceList };
      })
      .sort((a, b) => GROUPS.findIndex((g) => g.key === a.key) - GROUPS.findIndex((g) => g.key === b.key));
    return { module: { key: mod.key, label: mod.label, namespace: mod.namespace }, groups };
  });
}
...
const resourceOf = (key) => {
  const parts = key.split('.');
  if (parts[1] === 'knowledge' && (parts[3] || '').includes('knowledge_article')) return 'knowledge_article';
  if (parts[1] === 'request_catalog' && parts[3] === 'request') return 'request';
  if (parts[1] === 'request_catalog' && parts[3] === 'requested_item') return 'requested_item';
...
const defaultScopesFor = (key) => {
  ...
  if (['create'].includes(action)) return ['TENANT', 'DEPARTMENT', 'TEAM', 'OWN', 'REQUESTED_BY_ME'];
  if (['delete', 'export', 'archive', 'restore', 'view_audit', 'view_history', 'approve', 'reject', 'override', 'bulk_update'].includes(action)) {
    return ['TENANT', 'DEPARTMENT', 'TEAM'];
  }
  return ['TENANT', 'DEPARTMENT', 'TEAM', 'ASSIGNED_TO_ME', 'ASSIGNMENT_GROUP', 'OWN', 'REQUESTED_BY_ME'];
};
