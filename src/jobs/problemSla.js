...
async function checkProblemSlas() {
  ...
  for (const problem of problems) {
    ...
    if (ageHours > 168 && problem.priority !== 'Emergency') {
      ...
      problem.timeline.push({ at: now, by: 'system', message: 'Auto-escalated to Emergency after 7 days' });
      await problem.save();
      escalated++;
    } else if (ageHours > 72 && problem.priority === 'Low') {
      problem.priority = 'Normal';
      problem.timeline.push({ at: now, by: 'system', message: 'Auto-escalated to Normal after 3 days' });
      await problem.save();
      escalated++;
    }
  }
