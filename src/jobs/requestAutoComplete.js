...
async function autoCompleteRequests() {
  ...
  // Find open requests
  const openReqs = await Request.find({ status: { $in: ['open', 'work_in_progress'] }, isDeleted: false });
  let closedCount = 0;

  for (const req of openReqs) {
    const ritms = await RequestedItem.find({ requestId: req._id, isDeleted: false });
    if (!ritms.length) continue;

    const allClosed = ritms.every(r => ['closed_complete', 'closed_incomplete', 'closed_canceled'].includes(r.status));
    if (allClosed) {
  ...
  if (closedCount > 0) console.log(`[requestAutoComplete] auto-closed ${closedCount} requests`);
  return { closedCount };
}
...
function start() {
  ...
  cron.schedule('*/15 * * * *', async () => {
    try { await autoCompleteRequests(); } catch (e) { console.error('[requestAutoComplete]', e.message); }
  });
