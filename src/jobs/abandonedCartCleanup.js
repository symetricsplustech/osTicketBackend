...
async function cleanupAbandonedCarts() {
  ...
  const result = await Cart.updateMany(
    ...
    { $set: { status: 'abandoned', abandonedAt: new Date(), abandonedReason: 'auto_cleanup' } },
  );
  if (result.modifiedCount > 0) {
    console.log(`[abandonedCartCleanup] archived ${result.modifiedCount} inactive carts`);
  }
  return result;
}
...
function start() {
  ...
  cron.schedule('0 3 * * *', async () => {
    try { await cleanupAbandonedCarts(); } catch (e) { console.error('[abandonedCartCleanup]', e.message); }
  });
