...
const run = async () => {
  const email = (arg("email", "root@platform.local") || "")
    .toLowerCase()
    .trim();
  const password = arg(
    "password",
  ...
  if (!email || !password) {
    console.error(
      "Usage: node scripts/ensure-admin.js --email=<email> --password=<password> [--platform-role=<role>] [--permissions=a,b]",
    );
    process.exit(2);
  }
  ...
  if (!PLATFORM_ROLES.includes(platformRole)) {
    console.error(
      `Invalid --platform-role. Choose: ${PLATFORM_ROLES.join(", ")}`,
    );
    process.exit(2);
  }
