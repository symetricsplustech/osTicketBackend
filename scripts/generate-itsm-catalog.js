...
const source = path.resolve(__dirname, '../../SERVICENOW_ITSM_GRANULAR_PERMISSIONS_FROM_TECHNICAL_MASTER.md');
...
for (const line of markdown.split(/\r?\n/)) {
  ...
  if (heading) {
    ...
    current = number >= 1 && number <= 10 ? number : null;
    continue;
  }
  if (!current) continue;
...
const data = Object.fromEntries(
  Object.entries(modules).sort(([a], [b]) => a.localeCompare(b)).map(([key, values]) => [key, [...values].sort()]),
);
const total = Object.values(data).reduce((sum, values) => sum + values.length, 0);
...
console.log(`Generated ${total} permissions across ${Object.keys(data).length} modules.`);
