...
test("SAML service rejects tampered/unsigned response", async () => {
  const { validateSamlResponse } =
    await import("../src/services/saml.service.js");
  const xml = Buffer.from("<Response></Response>").toString("base64");
  const r1 = await validateSamlResponse({
    samlResponse: xml,
    idpCertificate: null,
  });
  assert.equal(r1.valid, false);
  const r2 = await validateSamlResponse({
    samlResponse: xml,
    idpCertificate: "x",
  });
  assert.equal(r2.valid, false);
  assert.ok(
    ["no_idp_certificate_configured", "invalid_signature"].includes(r2.reason),
  );
});
